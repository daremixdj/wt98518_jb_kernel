...
public class GeoCodingQuery implements IGeoCodingQuery {
    ...
    private  static String HwDataPath="/system/data/Met_areaInq.dat" ;
	private static final String[] FILTER_LIST = { "+86", "193", "10193", "1255577",
		"12593", "17901", "17908", "17909", "17911", "17931", "17951", "17969" };
	public static native String  queryPhoneUnicodeInformation(int index, int In3_7 ,String path);
	public static native String  queryTelUnicodeInformation(String in,String path);
		static{
		try
		{
			if (FeatureOption.WT_LOCATION_SUPPORT == true)
			{
			 System.loadLibrary("hwarea_jni"); 
		 }
		}
		catch(UnsatisfiedLinkError ule)
		{
			Log.d("GeoCodingQuery", "Load engmodelJNI failed");
		}
	}
...
private static String filtrate(String number) {
			boolean hasReplaced;
			do {
				hasReplaced = false;
				for (String num : FILTER_LIST) {
					if (number.startsWith(num)) {
						number = number.substring(num.length());
						hasReplaced = true;
					}
				}
			} while (hasReplaced);
...
public String queryAreaFromHwData(String In)
	{		 
			   int j1;
			 int j2;
			 int j3_7;
	    ...
	    if(In ==null)
			 ...
			 Alllen = In.length();
			  //Log.d("queryAreafromData", "In ="+In);	  
			 if( Alllen < 3)
			 {
			   return bret; 
			 }
			 Phonelen = Alllen>7 ?7:Alllen;
			 try
			 {
			 j1 = Integer.valueOf(In.substring(0,1));
			}catch (Exception e) {
				return bret;

			}
			try{
			 j2 = Integer.valueOf(In.substring(1,2));
			 }catch (Exception e) {
				return bret;
				
			}
			try{
			 j3_7 =Integer.valueOf(In.substring(2,Phonelen));
			 }catch (Exception e) {
				return bret;
				
			} 
			 //Log.d("queryAreafromData", "j1 ="+j1);
			 //Log.d("queryAreafromData", "j2 ="+j2);
			 //Log.d("queryAreafromData", "j3 ="+j3_7);
		try
		{
			 if(j1 ==1 &&(j2 ==3 || j2 ==4 ||j2 ==5||j2==7 ||j2==8))
			 {
			    //Log.d("queryAreafromData", "Alllen ="+Alllen);
				if( Alllen >10)
				{
				bret =queryPhoneUnicodeInformation(j2,j3_7,HwDataPath);
			  }
			  //Log.d("queryAreafromData", "Alllen ="+bret);
			}
			else if( j1 ==0)
			{
				if( In.length()>=4)
				{
					 bret  =  queryTelUnicodeInformation(In.substring(0,4),HwDataPath);
					   if(bret == null)
					   {
						 bret  =  queryTelUnicodeInformation(In.substring(0,3),HwDataPath);
					   }
				}
				else
				{
		...
		{
			Log.d("UnsatisfiedLinkError", "LoadqueryAreaFromHwData failed");
		}
			 Log.d("bret", "bret ="+bret);
			if(bret != null)
			{
				  int len =bret.length();
				  int npos = 0;
				  try
				  {
				  npos = Integer.valueOf(bret.substring(0,1));
				
				}catch (Exception e) {
				return bret;
	 			
			  }
					 Log.d("bret", "npos ="+npos);
				  bret = bret.substring(2,len);
    ...
    public String queryByNumber(String number) {
        ...
        String countryIso = "";
			if (FeatureOption.WT_LOCATION_SUPPORT == true)
         	{
				 returnValue = queryAreaFromHwData(number);
				 if( returnValue != null)
				 {
				    return  returnValue ;
				  }
