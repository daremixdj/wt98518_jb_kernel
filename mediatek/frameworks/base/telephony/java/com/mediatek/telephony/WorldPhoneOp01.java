...
public class WorldPhoneOp01 extends Handler implements IWorldPhone {
    ...
    private static int FDD_STANDBY_TIMER = 30;
    ...
    public WorldPhoneOp01(Context context) {
        ...
        FDD_STANDBY_TIMER = Settings.Global.getInt(context.getContentResolver(), Settings.Global.WORLD_PHONE_FDD_MODEM_TIMER, 30);
        logd("FDD_STANDBY_TIMER = " + FDD_STANDBY_TIMER + "s");
    ...
    public WorldPhoneOp01(int op, Context context, Phone[] phone) {
        ...
        for (int i=0; i<PhoneConstants.GEMINI_SIM_NUM; i++) {
            mPhone[i] = phone[i];
            mCM[i] = ((PhoneBase)((PhoneProxy)phone[i]).getActivePhone()).mCM;
        ...
        if (mUserType == sType1User || mUserType == sType2User) {
            ...
            if (mRegion == REGION_DOMESTIC) {
                if (ModemSwitchHandler.getModem() == ModemSwitchHandler.MODEM_SWITCH_MODE_TDD) {
                    ...
                    logd("TDD modem, stop searching CMCC service");
                    ...
                    logd("FDD modem, RAT=2g, expecting CMCC service");
                    logd("[isAllowCampOn]- Camp on OK");
                    return true;
                }
                ...
                if (ModemSwitchHandler.getModem() == ModemSwitchHandler.MODEM_SWITCH_MODE_TDD) {
                    ...
                    logd("TDD modem, expecting CMCC service");
                    ...
                    logd("FDD modem, expecting CMCC service");
                    logd("[isAllowCampOn]- Camp on OK");
                    return true;
                }
            } else {
        ...
        if (mUserType == sType1User || mUserType == sType2User) {
            ...
            if (ModemSwitchHandler.getModem() == ModemSwitchHandler.MODEM_SWITCH_MODE_TDD) {
                ...
                if (mRegion == REGION_FOREIGN) {
                    if (!sWaitInFdd) {
                        sWaitInFdd = true;
                        logd("Wait " + FDD_STANDBY_TIMER + "s");
                        /*if (sPowerManager.isScreenOn() == false) {
                            logd("Screen off -> change CREG to long format");
                            mPhone[m3gSimSlot].invokeOemRilRequestStrings(new String[]{"AT+CREG=2",""}, null);
                        }*/
                        postDelayed(mFddStandByTimerRunnable, FDD_STANDBY_TIMER*1000);
                    } else {
                        logd("Timer already set:" + FDD_STANDBY_TIMER +"s");
                    }
                } else {
    ...
    private Runnable mFddStandByTimerRunnable = new Runnable() {
        public void run() {
            ...
            if (isNoService()) {
                ...
                removeModemStandByTimer();
                logd("Not no-service, other PLMN exist -> stay in FDD modem");
            }
        }
    };
    ...
    private void removeModemStandByTimer() {
        ...
        if (sWaitInFdd) {
            ...
            sWaitInFdd = false;
            /*if (sPowerManager.isScreenOn() == false) {
                logd("Screen off -> change CREG to short format");
                mPhone[m3gSimSlot].invokeOemRilRequestStrings(new String[]{"AT+CREG=1",""}, null);
            }*/
    ...
    private boolean isNoService() {
        ...
        return !(mPhone[m3gSimSlot].getServiceState().isEmergencyOnly());*/
    }

    private void searchForDesignateService(String strPlmn) {
        logd("[searchForDesignateService]+ Search for CMCC srvice");
        if (strPlmn == null) {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
                ...
                if (m3gSimSlot != AUTO_SELECT_DISABLE) {
                    m3gSimSlot = intent.getIntExtra(GeminiPhone.EXTRA_3G_SIM, 0);
                }
                handle3gSwitched();
                ...
                if (serviceState != null) {
                    ...
                    mRilRadioTechnology = serviceState.getRilRadioTechnology();
    ...
    public void setRatMode(int ratMode, int slotId) {
