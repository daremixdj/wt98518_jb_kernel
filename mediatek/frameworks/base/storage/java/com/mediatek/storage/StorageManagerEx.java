...
/*add for "defaultpath is changed to phone storage after UMS or format SD" by cuiyingying 20140320 begin*/
import android.os.storage.IMountService;
import android.os.ServiceManager;
import android.os.RemoteException;
/*add for "defaultpath is changed to phone storage after UMS or format SD" by cuiyingying 20140320 end*/
...
public class StorageManagerEx implements IStorageManagerEx {
    ...
    /*add for "defaultpath is changed to phone storage after UMS or format SD" by cuiyingying 20140320 begin*/
    private static final String PROP_USER_SELECT_PATH = "persist.sys.user.select.path";
    ...
    public static String getDefaultPath() {
        ...
        /*add for "defaultpath is changed to phone storage after UMS or format SD" by cuiyingying 20140320 begin*/
        boolean swap = getSdSwapState();
        String sdpath = STORAGE_PATH_SD2;
        String phonepath = STORAGE_PATH_SD1;
        IMountService mountService = IMountService.Stub.asInterface(ServiceManager
                    .getService("mount"));

		if (FeatureOption.MTK_SHARED_SDCARD && !FeatureOption.MTK_2SDCARD_SWAP) {
			phonepath = STORAGE_PATH_SHARE_SD;
		} else if (swap) {
            sdpath = STORAGE_PATH_SD1;
            phonepath = STORAGE_PATH_SD2;
        }
		
        if (mountService != null){
            if(getNewDefaultPath().equals("SD")){
                try {
                    String volumeState = mountService.getVolumeState(getExternalStoragePath());
                    ...
                    Log.i(TAG, "Failed to get volume state", e);
                }
    ...
    /*add for "defaultpath is changed to phone storage after UMS or format SD" by cuiyingying 20140320 begin*/
    public static void setNewDefaultPath(String path) {
        Log.i(TAG, "setNewDefaultPath path=" + path);
        if (path == null) {
            Log.e(TAG, "setNewDefaultPath error! path=null");
            return;
        }

        try {
            SystemProperties.set(PROP_USER_SELECT_PATH, path);
        } catch (IllegalArgumentException e) {
            Log.e(TAG, "IllegalArgumentException when set new default path:" + e);
        }
    }

    public static String getNewDefaultPath() {
        String path = "";
        
        try {
            path = SystemProperties.get(PROP_USER_SELECT_PATH);
            Log.i(TAG, "new path=" + path);
        } catch (IllegalArgumentException e) {
            Log.e(TAG, "IllegalArgumentException when get new default path:" + e);
        }

        if (path.equals("")) {
            Log.i(TAG, "getNewDefaultPath empty! set to default.");
            setNewDefaultPath("SD");
            path = "SD";
        }
        Log.i(TAG, "getNewDefaultPath path=" + path);
        return path;
    }
    /*add for "defaultpath is changed to phone storage after UMS or format SD" by cuiyingying 20140320 end*/
}
