...
public final class MediatekClassFactory {
    ...
    public static <T> T createInstance(Class<?> clazz, Object... args) {
        ...
        } else if (clazz == IWorldPhone.class) {
            Object worldPhonePolicyObj = null;
            try {
                Log.i(TAG, "IWorldPhone init args length : " + args.length +
                    ", args[0] : " + args[0]);
                if (args.length > 0) {
                    if (args[0].equals(IWorldPhone.POLICY_OM)) {
                       worldPhonePolicyObj = getInstanceHelper("com.mediatek.telephony.WorldPhoneOm", args);
                        Log.i(TAG, "IWorldPhone newInstance (OM) : " + worldPhonePolicyObj);
                    } else if (args[0].equals(IWorldPhone.POLICY_OP01)) {
                        worldPhonePolicyObj = getInstanceHelper("com.mediatek.telephony.WorldPhoneOp01", args);
                        Log.i(TAG, "IWorldPhone newInstance (OP01) : " + worldPhonePolicyObj);
                    }
                }
                if (worldPhonePolicyObj == null) {
                    Log.i(TAG, "IWorldPhone newInstance (ERROR) : create request not mapped to any implementation.");
                }
            } catch (Exception e) {
                Log.e(TAG, "createInstance:got exception IWorldPhone");
                e.printStackTrace();
            }
            obj = worldPhonePolicyObj;
        } else {
