...
//#define FM36_ADB_WRITE_READ_SUPPORT
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT //for FM36
#include "fm36.h"
#endif
...
*******************************************************************************/
#ifdef FM36_ADB_WRITE_READ_SUPPORT
typedef struct
{
    int regaddr;
    int regdata;
...
fm36_REG_struct adb_show_param_buff[300]={0};
static int fm36_adb_write_length=0;

static struct proc_dir_entry *fm36_proc_entry;
unsigned char fm36_debug_cmd[64];
...
static DEFINE_MUTEX(ULmemUsedCouter_mutex);
static int ULmemUsedCouter  = 0;
...
static int AudDrv_Read_Proc_ExtCodec(char *buf, char **start, off_t offset, int count , int *eof, void *data)
{
    int len = 0;
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
    len += sprintf(buf + len , "WM5102_MODE = 0x%x\n", ExtCodec_ReadReg(WM5102_MODE));
    len += sprintf(buf + len , "WM5102_MIXING = 0x%x\n", ExtCodec_ReadReg(WM5102_MIXING));
    len += sprintf(buf + len , "WM5102_MUTE = 0x%x\n", ExtCodec_ReadReg(WM5102_MUTE));
    len += sprintf(buf + len , "WM5102_VOLA = 0x%x\n", ExtCodec_ReadReg(WM5102_VOLA));
    len += sprintf(buf + len , "WM5102_VOLB = 0x%x\n", ExtCodec_ReadReg(WM5102_VOLB));
    len += sprintf(buf + len , "WM5102_RAMP = 0x%x\n", ExtCodec_ReadReg(WM5102_RAMP));
    len += sprintf(buf + len , "WM5102_MISC1 = 0x%x\n", ExtCodec_ReadReg(WM5102_MISC1));
    len += sprintf(buf + len , "WM5102_MISC2 = 0x%x\n", ExtCodec_ReadReg(WM5102_MISC2));
#endif
    return len;
}
...
void Auddrv_DL_Interrupt_Handler(void)  // irq1 ISR handler
{
    ...
    if (Afe_Block->u4DataRemained < Afe_consumed_bytes || Afe_Block->u4DataRemained <= 0 || Afe_Block->u4DataRemained  > Afe_Block->u4BufferSize || AudIrqReset)
    {
        ...
        Afe_Block->u4DMAReadIdx  = HW_memory_index % Afe_Block->u4BufferSize;
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT //Qlw 2013/12/12
    ExtCodecDevice_Suspend();
#endif
    ...
    switch (cmd)
    {
        ...
        case START_MEMIF_TYPE:
        {
                ...
                MEM_DAI == MEM_Type || MEM_MOD_DAI == MEM_Type)
            {
                AFE_MEM_CONTROL_T *pAfe_MEM_ConTrol;
                pAfe_MEM_ConTrol = Auddrv_Get_MemIF_Context(arg);  
                mutex_lock(&ULmemUsedCouter_mutex);                                
                if(pAfe_MEM_ConTrol->flip == NULL)
                {                    
                    if(ULmemUsedCouter == 0)
                    {
                disable_dpidle_by_bit(MT_CG_AUDIO_AFE);
                ...
                mutex_unlock(&ULmemUsedCouter_mutex);                    
        ...
        case STANDBY_MEMIF_TYPE:
        {
                ...
                MEM_DAI == MEM_Type || MEM_MOD_DAI == MEM_Type)
            {
                ...
                pAfe_MEM_ConTrol = Auddrv_Get_MemIF_Context(arg);  
                mutex_lock(&ULmemUsedCouter_mutex);                                
                if(pAfe_MEM_ConTrol->flip == fp && ULmemUsedCouter > 0)
                {
                    ULmemUsedCouter--;    
                    if(ULmemUsedCouter == 0)
                    {
        ...
        case SET_EXTCODEC_OFF:
            mutex_lock(&gextcodec_mutex);
            fm36_voice_processor_Bypass();	//qhl 2013/11/12
            ExtCodec_PowerOff(arg);
            mutex_unlock(&gextcodec_mutex);
            break;
        ...
        case SET_EXTCODEC_VOICECALL_EARPIECE_OUT:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_voice_processor_16k_hs_noise_suppression(TRUE); //qhl 2013/11/12
            ExtCodec_Set_Voicecall_Earpiece_Out();
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_VOICECALL_HEADPHONE_OUT:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_voice_processor_VoiceInCall_HeadPhone(); //qhl 2013/11/12
            //fm36_voice_processor_Bypass();
            ExtCodec_Set_Voicecall_Headphone_Out();
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_VOICECALL_SPEAKER_OUT:
        {
            mutex_lock(&gextcodec_mutex);
            if(ExtCodec_Get_Factory_MicState())
            {
                fm36_voice_processor_Bypass();
            }
            else
            {
                fm36_voice_processor_16k_hf_noise_suppression(TRUE); //qhl 2013/11/12
            }
            ExtCodec_Set_Voicecall_Speaker_Out();
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_MUSIC_SPEAKER_OUT:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_voice_processor_Bypass();
            ExtCodec_Set_Music_Speaker_Out();
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_DUALMODE_OUT:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_voice_processor_Bypass();
            ExtCodec_Set_DualMode_Out();
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_MIC1_IN:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_voice_processor_Bypass();
            ExtCodec_Set_Mic1_In();
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_MIC2_IN:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_voice_processor_Bypass();
            ExtCodec_Set_Mic2_In();
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_HPMIC_IN:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_voice_processor_Bypass();
            ExtCodec_Set_HpMic_In();
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case FM36_DENOISE_ENABLE:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_denoise_control(true);
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case FM36_DENOISE_DISABLE:
        {
            mutex_lock(&gextcodec_mutex);
            fm36_denoise_control(false);
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_FACTORY_MAIN_MIC:
        {
            mutex_lock(&gextcodec_mutex);
            ExtCodec_Factory_MainMic_En(arg);
            mutex_unlock(&gextcodec_mutex);
            break;
        }
        case SET_EXTCODEC_FACTORY_REF_MIC:
        {
            mutex_lock(&gextcodec_mutex);
            ExtCodec_Factory_RefMic_En(arg);
            mutex_unlock(&gextcodec_mutex);
            break;
        }
#endif
        ...
        case AUDDRV_ECHOREF_MODE:
        {
            ...
            xlog_printk(ANDROID_LOG_ERROR, "Sound", "AUDDRV_ECHOREF_MODE= %lu\n", arg);
            bEchoRefMode = arg != 0;
...
static s32 fm36debug_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
    u64 ret = 0;
    uint8 i = 0;
    uint16 reg = 0;
    uint16 value = 0;
    unsigned char fm36_debug_buffer[3072]={0};
    memset(fm36_debug_cmd, 0, sizeof(fm36_debug_cmd));

    ret = copy_from_user(&fm36_debug_buffer, buff, len);
    if (ret)
    {
        xlog_printk(ANDROID_LOG_DEBUG, __func__,"fm36_debug:copy_from_user failed.");
    }
	 xlog_printk(ANDROID_LOG_DEBUG, __func__,"fm36_debug:%s",fm36_debug_buffer);

   if(strcmp(fm36_debug_buffer,"reset")==0)
   	{
		fm36_reset();
		xlog_printk(ANDROID_LOG_DEBUG, __func__,"reset ok!");
		return len;
   	}
   char buffer[10]={0};
   memcpy(buffer,fm36_debug_buffer,8);
   buffer[8]='\0';
      if(strcmp(buffer,"only_one")==0)
   	{
   	    fm36_adb_write_length=(strlen(fm36_debug_buffer)-1)/8-1;
		memset((char*)adb_show_param_buff,0,sizeof(adb_show_param_buff));
	   	 for(i=1;i<fm36_adb_write_length+1;i++)
	    	{
		    memcpy(fm36_debug_cmd,fm36_debug_buffer+i*8,8);
		    str2int8(fm36_debug_cmd);
		    reg = ((data_buffer[0]<<8)|data_buffer[1]);
		    value = ((data_buffer[2]<<8)|data_buffer[3]);
			
		    adb_show_param_buff[i-1].regaddr=reg;
		    adb_show_param_buff[i-1].regdata=0;
		    xlog_printk(ANDROID_LOG_DEBUG, __func__, "fm36_debug write reg:0x%04x, value:0x%04x\n", reg, value);

		    fm36_write_word( reg,  value);
	    	}
		xlog_printk(ANDROID_LOG_DEBUG, __func__,"only_one");
		return len;
   	}

    fm36_adb_write_length=(strlen(fm36_debug_buffer)-1)/8;
   fm36_reset();
	memset((char*)adb_show_param_buff,0,sizeof(adb_show_param_buff));
    for(i=0;i<fm36_adb_write_length;i++)
    	{
	    memcpy(fm36_debug_cmd,fm36_debug_buffer+i*8,8);
	    str2int8(fm36_debug_cmd);
	    reg = ((data_buffer[0]<<8)|data_buffer[1]);
	    ...
	    adb_show_param_buff[i].regaddr=reg;
	    adb_show_param_buff[i].regdata=0;
	    xlog_printk(ANDROID_LOG_DEBUG, __func__, "fm36_debug write reg:0x%04x, value:0x%04x\n", reg, value);
...
static s32 fm36debug_read(char *buf, char **start, off_t offset, int count , int *eof, void *data)
{

    int len = 0;
    int i;
     xlog_printk(ANDROID_LOG_DEBUG , __func__,"fm36_debug read fm36_adb_write_length:0x%04x\n",fm36_adb_write_length);
     fm36_adb_read( (char*)&adb_show_param_buff,fm36_adb_write_length);
    for ( i = 0; i <fm36_adb_write_length; i++) {
		  len += sprintf(buf + len , "reg = 0x%04x, value:0x%04x\n",adb_show_param_buff[i].regaddr, adb_show_param_buff[i].regdata);
 		  xlog_printk(ANDROID_LOG_DEBUG , __func__,"fm36_debug read reg:0x%04x, value:0x%04x\n", adb_show_param_buff[i].regaddr, adb_show_param_buff[i].regdata);
	}
...
static int AudDrv_mod_init(void)
{
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT //for FM36
    CLK_Monitor(1,0x1,1); ////CLKM1£¬0x1Ê±ÖÓÔ´26M£¬1=div
    FM36_Register();
#endif
    ...
    create_proc_read_entry("ExtCodec",
                           ...
                           NULL);
#ifdef FM36_ADB_WRITE_READ_SUPPORT 
    fm36_proc_entry = create_proc_entry("fm36_debug", 0666, NULL);	 
    if (fm36_proc_entry == NULL)    
    {		
        xlog_printk(ANDROID_LOG_INFO, "fm36",  "fm36_debug:Couldn't create proc entry!");		 
        //return FALSE;	 
    }	  
    else	  
    { 	   
        xlog_printk(ANDROID_LOG_INFO, "fm36","fm36_debug:Create proc entry success!");	
        fm36_proc_entry->write_proc = fm36debug_write;
       fm36_proc_entry->read_proc = fm36debug_read;
        //return TRUE;
    }
#endif
