...
#define MHALL_LOG(fmt, args...)	printk(KERN_ERR MHALL_TAG "%s(%d):" fmt, __FUNCTION__, __LINE__, ##args)
#define MHALL_DBG(fmt, args...)	printk(KERN_ERR fmt, ##args)
#else
#define MHALL_FUN(f)
...
/****************************************************************/
static void do_pwrkey_handle(int press)
{    
	if(0 == press){
	...
	pmic_config_interface(INT_STATUS0,0x1,0x1,5);
}
...
{	
	...
	do_pwrkey_handle(1); // press	 pwrkey
	do_pwrkey_handle(0); // release  pwrkey
...
void mhall_eint_func(void)
{
	...
	if(NORMAL_BOOT == get_boot_mode()){
		if(mhall_is_enable){
			if((cur_mhall_eint_state == EINT_PIN_COVER)&&(mhall_resume_flag == 0)){	
				//MHALL_LOG("System is suspend, cover nothing to do!\n");
			}else if((cur_mhall_eint_state == EINT_PIN_LEAVE)&&(mhall_resume_flag == 1)){
				//MHALL_LOG("System is resume, leave nothing to do!\n");
			}else{
				schedule_work(&mhall_eint_work);
			}
		}else{	// Mhall is disable!
		}
	}else{	// Nothing to do !
	}
}
