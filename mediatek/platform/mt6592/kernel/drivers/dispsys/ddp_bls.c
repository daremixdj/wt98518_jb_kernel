...
#if (defined(LED_OUTPUT_MAX_LEVEL) && defined(LED_OUTPUT_MIN_LEVEL))	
	level = ((LED_OUTPUT_MAX_LEVEL - LED_OUTPUT_MIN_LEVEL)*level + (255*LED_OUTPUT_MIN_LEVEL - 30*LED_OUTPUT_MAX_LEVEL)*4)/(255 - 30);
#endif

#if defined(LED_OUTPUT_LEVEL_PERCENT)
	level=(level*LED_OUTPUT_LEVEL_PERCENT)/100;
    if (level<LED_OUTPUT_MIN_LEVEL*4)
    {
      level=(LED_OUTPUT_MIN_LEVEL*4);
    }
#endif
