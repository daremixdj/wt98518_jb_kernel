...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT //Qlw 2013/11/13
...
status_t AudioFMResourceManager::SetFmDirectConnection(const bool enable)
{
    ...
    if (enable == true)
    {
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT //Qlw 2013/11/01
        mAudioDigitalInstance->SetinputConnection(AudioDigitalType::Connection, AudioDigitalType::I12, AudioDigitalType::O00);
        mAudioDigitalInstance->SetinputConnection(AudioDigitalType::Connection, AudioDigitalType::I13, AudioDigitalType::O01);
#else
        ...
        {
            mAudioDigitalInstance->Set2ndI2SOutAttribute(GetFmDownlinkSamplingRate());//需要仿照SetI2SDacOutAttribute去重写Set2ndI2SOutAttribute
        }
        mAudioDigitalInstance->Set2ndI2SOutEnable(true);
#else
        if (AudioMTKStreamManager::getInstance()->IsOutPutStreamActive() == false)
    ...
    else
    {
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT //Qlw 2013/11/01
        mAudioDigitalInstance->SetinputConnection(AudioDigitalType::DisConnect, AudioDigitalType::I12, AudioDigitalType::O00);
        mAudioDigitalInstance->SetinputConnection(AudioDigitalType::DisConnect, AudioDigitalType::I13, AudioDigitalType::O01);
#else
        ...
        {
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT //Qlw 2013/11/01
            mAudioDigitalInstance->Set2ndI2SOutEnable(false); //这里的函数调用，应该是mAudioDigitalInstance->Set2ndI2SOutEnable(false); 
#else
