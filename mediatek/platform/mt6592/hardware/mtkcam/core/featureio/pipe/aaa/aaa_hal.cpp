...
MBOOL Hal3A::postCommand(ECmd_T const eCmd, MINT32 const i4Arg)
{
    ...
    {
        MY_LOG_IF(sm_3AThreadLogEnable, "3A PreviewEnd, start wait semAFProcEnd");
        //Once 3A receive PreviewEnd, 3A cannot return until AF thread also gets AFirq and finish this frame
        MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::postCommand] start waiting semAFProcEnd semaphore\n");
    ...
    if (eCmd == ECmd_Update)
    {
        ...
        {
            int Val;
            MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::postCommand] start waiting mAFMtx lock\n");
            Mutex::Autolock autoLock(mAFMtx); //lock for critical section
            MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::postCommand] get mAFMtx lock\n");
            if (g_semAFIRQWait == 0)
            {
...
MVOID * Hal3A::AFThreadFunc(void *arg)
{
    ...
    while (g_bAFThreadLoop) {
        //
        MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] start waiting mAFMtx lock (1)\n");
        {Mutex::Autolock autoLock(_this->mAFMtx); g_semAFIRQWait = 0;}
        MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] get mAFMtx lock (1)\n");
        ...
        MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] get mAFMtx lock (2)\n");
#ifdef MTK_SLOW_MOTION_VIDEO
        ...
        else
        {
            ...
            {
                int Val2;
                MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] start waiting mAFMtx lock (3)\n");
                Mutex::Autolock autoLock(_this->mAFMtx);
                MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] get mAFMtx lock (3)\n");
