...
#include "emi.h"//wingtech wangfuqiang add for get EMMC flash info
...
static char emmc_nand_device_name[30]="SD7DP24F_4G";
extern int num_of_emi_records;
extern EMI_SETTINGS emi_settings[];
char * mmc_get_emmc_name(void)
{
	return (char *)emmc_nand_device_name;
}
    ...
    if (err == MMC_ERR_NONE && !boot_dev_found) {
		//wingtech wangfuqiang add for get EMMC flash info add begin 
			...
			if(buf[0] == 0x45){
				...
				*/
				printf("emmc nand device name %s found=%d\r\n",emmc_nand_device_name,found);
				
			}else{
		
			memset(emmcid, 0, emmc_nand_id_len);
			emmc_nand_id_len = emmc_nand_id_len > 16 ? 16 : emmc_nand_id_len;
			memcpy(emmcid, buf, emmc_nand_id_len);
			if(fw_id_len == 6)
				memcpy(emmcid+emmc_nand_id_len,buf_sanid+32,6);
			else
				*(emmcid+emmc_nand_id_len) = buf[9];		
				...
				if (emi_settings[i].type != 0)
				{
					if ((emi_settings[i].type & 0xF00) != 0x000)
					{
						...
						{	/* valid ID */
							...
							{
								/* NAND */
								if (memcmp(emmcid, emi_settings[i].ID, emi_settings[i].id_length) == 0){
									memset(emmcid + emi_settings[i].id_length, 0, sizeof(emmcid) - emi_settings[i].id_length);
									mdl_number = i;
									found = 1;
									break; /* found */
								}
					    ...
					    {
							printf("[EMI] Fail to get flash ID !!\n");
					    }
					}
					else
					{
			...
			if(found == 1)
				memcpy(emmc_nand_device_name,emi_settings[mdl_number].devciename,30);
		
			printf("emmc nand device mdl_number=%d name=%s\r\n",mdl_number,emmc_nand_device_name);
			}		
