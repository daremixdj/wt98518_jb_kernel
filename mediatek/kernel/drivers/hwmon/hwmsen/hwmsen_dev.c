...
#if defined(MTK_AUTO_DETECT_GYROSCOPE)
static char gyro_name[25];
static struct sensor_init_info* gyro_init_list[MAX_CHOOSE_G_NUM] = {0};
#endif
/*----------------------------------------------------------------------------*/
struct dev_context {
    int		polling_running;
...
int hwmsen_gyro_remove(struct platform_device *pdev)
{
    //int err =0;
	int i=0;
	for(i = 0; i < MAX_CHOOSE_G_NUM; i++)
	{
	   if(0 ==  strcmp(gyro_name,gyro_init_list[i]->name))
	   {
	      if(NULL == gyro_init_list[i]->uninit)
	      {
	        HWM_LOG(" hwmsen_gyro_remove null pointer \n");
	        return -1;
	      }
	      gyro_init_list[i]->uninit();
	   }
	}
    return 0;
}

static int gyro_sensor_probe(struct platform_device *pdev) 
{
    int i =0;
	int err=0;
	HWM_LOG(" gyro_sensor_probe +\n");
	for(i = 0; i < MAX_CHOOSE_G_NUM; i++)
	{
	  if(NULL != gyro_init_list[i])
	  {
	    err = gyro_init_list[i]->init();
		if(0 == err)
		{
		   strcpy(gyro_name,gyro_init_list[i]->name);
		   HWM_LOG(" gyro_sensor %s probe ok\n", gyro_sensor_name);
		   break;
		}
	  }
	}
...
static struct platform_driver gyro_sensor_driver = {
	.probe      = gyro_sensor_probe,
	.remove     = hwmsen_gyro_remove,    
	.driver     = 
	{
		.name  = "gyro",
//		.owner = THIS_MODULE,
...
int hwmsen_gyro_add(struct sensor_init_info* obj) 
{
    int err=0;
	int i =0;
	...
	for(i =0; i < MAX_CHOOSE_G_NUM; i++ )
	{
	    if(NULL == gyro_init_list[i])
	    {
	          gyro_init_list[i] = kzalloc(sizeof(struct sensor_init_info), GFP_KERNEL);
		  if(NULL == gyro_init_list[i])
		  {
		     HWM_ERR("kzalloc error");
		     return -1;
		  }
		  obj->platform_diver_addr = &gyro_sensor_driver;
	          gyro_init_list[i] = obj;
...
EXPORT_SYMBOL_GPL(hwmsen_gyro_add);
...
#if defined(MTK_AUTO_DETECT_GYROSCOPE)
		if(platform_driver_register(&gyro_sensor_driver))
		{
			HWM_ERR("failed to register gyro_sensor driver");
			return -ENODEV;
		}
#endif
	return 0;
