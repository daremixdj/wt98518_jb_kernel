...
static BOOL disp_drv_init_context(void)
{
    if (disp_if_drv != NULL && lcm_drv != NULL){
		hardwareinfo_set_prop(HARDWARE_LCD, disp_lcm_name);
        return TRUE;
    }
...
// Fence Sync Object
    ...
    else
    {
        ...
        if(rdma_realtime_layer != -1)
        {
            ...
            unsigned long flags;
            spin_lock_irqsave(&buf_usage_lock, flags);
            buf_idx_using[1][rdma_realtime_layer] = captured_layer_config[rdma_realtime_layer].buff_idx;
