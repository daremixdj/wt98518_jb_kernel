...
#define bat_temp_switch  "bat_temp_switch"
unsigned int g_battery_temp_switch_enable = 0;
...
extern kal_int32 call_wake_flag ; // Bug269421,huangfusheng.wt,add,20140415 battery gauge is incorrect during talking battery life
...
#if (WT_CHARGING_LOWTEMP_SUPPORT)
kal_bool Low_Temper_Charging_Flag = KAL_FALSE;
kal_uint8 low_temper_charging_count = 0;
kal_uint8 normal_temper_charging_count = 0;
#define LOW_BATTEMP_CHECK_COUNT 2

static void BMT_PhyCheck_CheckTempRange(void)
{
		
	if(Low_Temper_Charging_Flag == KAL_FALSE)	
	{
	   if(BMT_status.temperature < 0 && BMT_status.temperature > -10)//-10 ~ 0
	   {               			
	      low_temper_charging_count++;		
	   }
	   else
	   {			
	   		low_temper_charging_count = 0;		
	   }

	   if(low_temper_charging_count >= LOW_BATTEMP_CHECK_COUNT)		
	   	{			
	   		low_temper_charging_count = 0;			
			Low_Temper_Charging_Flag = KAL_TRUE;		
		 }
	  }

	  else
	  {
		if(BMT_status.temperature < 50 && BMT_status.temperature >= 0)//0 ~ 50
		{			
			normal_temper_charging_count++;		
		}
		else		
		{			
			normal_temper_charging_count = 0;		
		}

		if(normal_temper_charging_count >= LOW_BATTEMP_CHECK_COUNT)
		{			
			normal_temper_charging_count = 0;			
			Low_Temper_Charging_Flag = KAL_FALSE;					
		}
...
{ 
	...
	if((bat_meter_timeout == KAL_TRUE) || (call_wake_flag == KAL_TRUE)) // Bug269421,huangfusheng.wt,modify,20140415 battery gauge is incorrect during talking battery life
	{
		SOC = battery_meter_get_battery_percentage();
		bat_meter_timeout = KAL_FALSE;
		call_wake_flag = KAL_FALSE; // Bug269421,huangfusheng.wt,add,20140415 battery gauge is incorrect during talking battery life
	}
	else
	{
		if (previous_SOC == -1)
...
{
	//@hfs---start
	if(g_battery_temp_switch_enable != 1)
	{
...
#define BATTERY_TEMP_SWITCH 					0X99
#define SET_BATTERY_TEMP_SWITCH_ENABLE		    _IOW(BATTERY_TEMP_SWITCH, 0x01, int)
#define GET_BATTERY_TEMP_SWITCH_STATUS			_IOR(BATTERY_TEMP_SWITCH, 0x02, int)

static int bat_temp_switch_open(struct inode *inode, struct file *file)
{
	return 0;
}

static int bat_temp_switch_release(struct inode *inode, struct file *file)
{
	return 0;
}

static int bat_temp_switch_ioctl(struct file *file,
								unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	 ...
	 case SET_BATTERY_TEMP_SWITCH_ENABLE:
		...
		battery_xlog_printk(BAT_LOG_CRTI, "******** @hfs bat_temp_switch_ioctl enable :%d ********\n", enable);

			if(enable)
				{
				  g_battery_temp_switch_enable=1;
				}
			else
				{
				  g_battery_temp_switch_enable=0;
				}
		
		battery_xlog_printk(BAT_LOG_CRTI, "******** g_battery_temp_switch_enable:%d ********\n", g_battery_temp_switch_enable);
		
		break;
		

	case GET_BATTERY_TEMP_SWITCH_STATUS:
		
		enable = g_battery_temp_switch_enable;
		
		battery_xlog_printk(BAT_LOG_CRTI, "******** @hfs GET_BATTERY_TEMP_SWITCH_STATUS:enable %d ********\n",enable);
...
static struct file_operations bat_temp_switch_fops = {
	.owner           = THIS_MODULE,		
	.open            = bat_temp_switch_open,
	.release         = bat_temp_switch_release,
	.unlocked_ioctl  = bat_temp_switch_ioctl,
};

static struct miscdevice bat_temp_switch_dev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = bat_temp_switch,
	.fops = &bat_temp_switch_fops,
};
//@hfs---end
...
static int battery_probe(struct platform_device *dev)    
{
	...
	//@hfs--------start
	ret = misc_register(&bat_temp_switch_dev);
    if(ret){
