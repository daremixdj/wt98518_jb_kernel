...
#define FULL_CHECK_TIMES		6
 ...
 extern kal_bool Low_Temper_Charging_Flag;
#endif
...
#if (WT_CHARGING_LOWTEMP_SUPPORT)
		 if(KAL_TRUE==Low_Temper_Charging_Flag)
		 {
			 g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;
    ...
    else
    {
        ...
        {    
			...
			/*Set CV Voltage*/
                ...
                #endif            
	#if (WT_CHARGING_LOWTEMP_SUPPORT)	
					if(Low_Temper_Charging_Flag == KAL_TRUE)
					{
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	kal_uint32 recharge_vol =RECHARGING_VOLTAGE;
	...
	/* enable/disable charging */
  	battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);
		...
		if(KAL_TRUE==Low_Temper_Charging_Flag)
		{
			recharge_vol = RECHARGING_VOLTAGE_LT;
		}
		else
		{
			recharge_vol = RECHARGING_VOLTAGE;
		}
	#endif
	
	if(BMT_status.bat_vol < recharge_vol) 
    {
