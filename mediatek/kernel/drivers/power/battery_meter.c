...
kal_int32 call_wake_flag = KAL_FALSE; // Bug269421,huangfusheng.wt,add,20140415 battery gauge is incorrect during talking battery life
// ============================================================ //
// function prototype
// ============================================================ //
...
{    
	...
	//@mtk patch ----start add by will 
#ifdef WT_BATTERY_RBAT_COMPERSATE
...
if(FACTORY_BOOT != get_boot_mode())
{
    if(g_rtc_fg_soc >= gFG_capacity_by_v)
    {
        if(((g_rtc_fg_soc != 0) && ((g_rtc_fg_soc-gFG_capacity_by_v) < CUST_POWERON_DELTA_CAPACITY_TOLRANCE) &&(( gFG_capacity_by_v > CUST_POWERON_LOW_CAPACITY_TOLRANCE || bat_is_charger_exist() == KAL_TRUE)))
			|| ((g_rtc_fg_soc != 0) &&(g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT || g_boot_reason == BR_TOOL_BY_PASS_PWK || g_boot_reason == BR_2SEC_REBOOT || ((g_boot_mode == RECOVERY_BOOT)&&((g_rtc_fg_soc-gFG_capacity_by_v) < 10))))) // Bug265764 huangfusheng.wt modify 20140415 SD UPDATE change battery SOC ERROR
            
        {
            gFG_capacity_by_v = g_rtc_fg_soc;            
        }
		else if((g_boot_mode == RECOVERY_BOOT)&&((g_rtc_fg_soc-gFG_capacity_by_v) >= 10))  // +Bug265764 huangfusheng.wt modify 20140415 SD UPDATE change battery SOC  ERROR
		{
			g_rtc_fg_soc = gFG_capacity_by_v;
			set_rtc_spare_fg_value(gFG_capacity_by_v);
		}   // -Bug265764 huangfusheng.wt modify 20140415 SD UPDATE change battery SOC ERROR
    }
    else
    {
    	if(((g_rtc_fg_soc != 0) && ((gFG_capacity_by_v-g_rtc_fg_soc) < CUST_POWERON_DELTA_CAPACITY_TOLRANCE) &&(( gFG_capacity_by_v > CUST_POWERON_LOW_CAPACITY_TOLRANCE || bat_is_charger_exist() == KAL_TRUE)))
			|| ((g_rtc_fg_soc != 0) &&(g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT || g_boot_reason == BR_TOOL_BY_PASS_PWK || g_boot_reason == BR_2SEC_REBOOT || ((g_boot_mode == RECOVERY_BOOT)&&((gFG_capacity_by_v-g_rtc_fg_soc) < 10))))) // Bug265764 huangfusheng.wt modify 20140415 SD UPDATE change battery SOC ERROR
        {
            gFG_capacity_by_v = g_rtc_fg_soc;            
        }

		else if((g_boot_mode == RECOVERY_BOOT)&&((gFG_capacity_by_v-g_rtc_fg_soc) >= 10)) // +Bug265764 huangfusheng.wt modify 20140415 SD UPDATE change battery SOC ERROR
		{
			g_rtc_fg_soc = gFG_capacity_by_v;
			set_rtc_spare_fg_value(gFG_capacity_by_v);
		} // -Bug265764 huangfusheng.wt modify 20140415 SD UPDATE change battery SOC ERROR
...
if(abs(gFG_capacity_by_v-g_rtc_fg_soc)>10&&abs(gFG_capacity_by_v-g_rtc_fg_soc)<20) 
gFG_capacity_by_v=(gFG_capacity_by_v+g_rtc_fg_soc)/2;	
...
void oam_run(void)
{
	...
	//@hfs .wingtech start 20140308 
	#ifdef WT_OAM_VBAT_COMPERSATE
	if(BMT_status.charger_exist == KAL_FALSE)
	{
		if((vol_bat - BMT_status.bat_vol)>100) 
		{
			vol_bat = BMT_status.bat_vol;
		}
	}
	#endif
	//@hfs---end
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	if((g_call_state == CALL_ACTIVE)&&(BMT_status.charger_exist == KAL_FALSE)) //+Bug269421,huangfusheng.wt,add,20140415 battery gauge is incorrect during talking battery life
	{ 

		
		ret_time = 150;
		call_wake_flag = KAL_TRUE ;
		  
		
	}    //- Bug269421,huangfusheng.wt,add,20140415 battery gauge is incorrect during talking battery life
...
//    kal_int32 vbat_diff = 200;
...
#endif        
    {
    	...
    	if(rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec > 3600)	 
    	{
