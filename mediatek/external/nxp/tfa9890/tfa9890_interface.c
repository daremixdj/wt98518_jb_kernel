...
//#define LOCATION_FILES "/sdcard/mtklog/"
#endif
...
void set_bypass_dsp_incall(int bypass)
{
...
void tfa9890_EchoReferenceConfigure(int config)
{
    ...
    if(config)
    {
       ...
       TFA_LOGD("tfa9890_EchoReferenceConfigure, error(%d)", err);
       assert(err == Tfa98xx_Error_Ok);
    }
    ALOGD("%s -, flag= %d",__func__,config);
}
...
EXPORT_SYMBOL(set_bypass_dsp_incall);
