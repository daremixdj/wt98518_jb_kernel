...
//@hfs---start
#define CHARGER_OVER_TEMP_HIGH 500
#define CHARGER_OVER_TEMP_LOW -100
#define CHARGER_TEMP_PATH "/sys/class/power_supply/battery/batt_temp"
//@hfs---end
#define IPO_FAST_REBOOT "sys.ipo.fast_reboot"
#define IPO_BKL_ON_DELAY "sys.ipo.bklondelay"

#define BATTERY_ID_PATH "/sys/class/power_supply/battery/batt_id"
/*
 ...
 *	   0: no	battery
 */
int is_realBatteryID()
{
	int id = get_int_value(BATTERY_ID_PATH);
...
//add by mtk for ALPS01278214
#ifdef WT_HTC_REQ
#define BACKLIGHT_PATH "/sys/class/leds/lcd-backlight/brightness"

void set_backlight_level()
{
    //backlight level can be modify
	set_str_value(BACKLIGHT_PATH, "50");
}

static void* backlight_thread_routine(void *arg)
{	
        ...
        SXLOGI("backlight_thread sleep for 30 sec");
        sleep(30);
        set_backlight_level();
        pthread_exit(NULL);
    	return 0;
...
void Change_backlight_to_half_trigger()
{
    int ret = 0;
    pthread_attr_t attr;
    ...
    pthread_attr_init(&attr);
	SXLOGI("pthread_create backlight thread.");
    ret = pthread_create(&backlight_thread, &attr, backlight_thread_routine, NULL);
    if (ret != 0) 
    {
        SXLOGE("create test pthread failed.\n");
    }
...
int status_cb(int event, int data1, int data2)
{
    ...
    switch (event) {
        ...
        case EVENT_DRAW_CHARGING_ANIM:
            ...
            #ifdef WT_HTC_REQ
            if(data1==1){
            	Change_backlight_to_half_trigger();
            }
			      #endif
            //add end
            if (data1 == 0)
...
//@hfs---start
int get_ovtemp_status()
{
    int temp = get_int_value(CHARGER_TEMP_PATH);
    SXLOGI("charger temp: %d\n",temp);

    if ((temp > CHARGER_OVER_TEMP_HIGH)||(temp < CHARGER_OVER_TEMP_LOW))
	{
        return 1;
    }
    return 0;
