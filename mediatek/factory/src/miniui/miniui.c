    ...
    if (iv->menu_items) {
        ...
        for (j = 1; i < iv->menu_top + iv->menu_items; ++i, ++j) {
            if (i == iv->menu_top + iv->menu_sel) {
                ...
                // sunxuewen modified for htc_cid begin
                //gr_color(120, 120, 255, 160);
                if(iv->m_items[i - iv->menu_top].itemtext != 0){
                	set_gr_color(iv->m_items[i - iv->menu_top].itemtext);
                }else{
                	gr_color(120, 120, 255, 160);
                }
               // sunxuewen modified for htc_cid end
                draw_menu_text(j, iv->menu[i]);
                gr_color(64, 96, 255, 160);
            }
	...
	if(1==is_full_mode){
		if(auto_select){
		    while (chosen_item<0 && !exit) {
				...
				LOGD(TAG "itemview_auto_select_get_cur_KEY: %d\n", key);
				if(KEY_VOLUMEUP == get_back_fail_key())
				{
			        switch (key) {
			            case UI_KEY_DOWN:
			            case UI_KEY_VOLDOWN:
		    ...
		    while (chosen_item<0 && !exit) {
				...
				LOGD(TAG "itemview_get_cur_KEY: %d\n", key);
