...
const char* dial112(const int fd)
{
    ...
    char *dia112_ok = "ok";
	int write_len = 0;
	int fd_backlight = -1,fd_suspend = -1;
	char *s_backlight_off = "0";
	char *s_backlight_on = "128";
...
//jin add for 112 current test  start	
	write_len = write(usb_com_port, dia112_ok, strlen(dia112_ok));
	LOGD(TAG "dial112 write data to pc\n");
	if(write_len != strlen(dia112_ok))
	{
		LOGD(TAG "dial112 write data to pc fail\n");
	}
    ...
    write_len = write(fd_backlight, s_backlight_off, strlen(s_backlight_off));
    ...
    write_len = write(fd_suspend, s_state_mem, strlen(s_state_mem));

	sleep(30); 

    write_len = write(fd_backlight, s_backlight_on, strlen(s_backlight_on));
	write_len = write(fd_suspend, s_state_on, strlen(s_state_on));
	close(fd_suspend);
	close(fd_backlight);
//jin add for 112 current test  end
