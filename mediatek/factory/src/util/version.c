...
void print_verinfo(char *info, int *len, char *tag, char *msg)
{
	...
	if(msg_len>max_len-tag_len-2) {
		_len += sprintf(info+_len,"\n ");//zhaoqf_sh modify 4 blank-space to 1
		g_nr_lines++;
	}

	while(msg_len>0) {
		buf_len = max_len - 1;// 4 --zhaoqf_sh modify 4 to 1
...
#if 1
		if(msg_len>0) {
			for(buf_len=0; buf_len < 1/* 4 */; buf_len++) buf[buf_len]=' ';// 4 --zhaoqf_sh modify 4 to 1
			buf[buf_len]=0;
...
int create_report(item_t *item, item_t *rpt_items, int maxitems, char *buf, int size)
{
    ...
    bool find_flag =false; //zhaoqf_sh
    //report_save_struct report_s[maxitems] = {0}; 
    int ret = 0;
    while (i < maxitems && item->name) {
    		ret = report_readByID(item->id);
    		if(ret == -1){
    			 ret = 0;
    		}
        ...
        if(!find_flag && mod->test_type==BACKGROUND_TEST){
            ptr = buf + len;
            len += sprintf(ptr, "====================================\n"); 
            ptr[len++] = '\0';
            rpt_items[i].id = ITEM_ID_SEPRATE_FLAG;//mod->id;
        ...
        if (mod && mod->visible && len < size) {
            ...
            len += sprintf(ptr, "[%c] %s ", 
                (ret >= FTM_TEST_MAX) ? 
                result[FTM_TEST_UNKNOWN] : result[ret], item->name);
            ptr[len++] = '\0';
            rpt_items[i].id = mod->id;
            rpt_items[i].name = ptr;
            rpt_items[i].background = (ret >= FTM_TEST_MAX) ?
                0 : bgc[ret];
            i++;
        }
        item++;
