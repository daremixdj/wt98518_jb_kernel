...
static void *emmc_update_iv_thread(void *priv)
{
 ...
 /*   while (1) {
    ...
    */
    //pthread_exit(NULL);
    	emmc_update_info(mc, mc->info);
        if(0 == is_full_mode) iv->redraw(iv);//zhaoqf_sh for full test
...
int emmc_entry(struct ftm_param *param, void *priv)
{
    ...
    {
    iv->set_items(iv, emmc_auto_items, 0);
    }
    else
    {
    ...
    if((1 == is_full_mode) || (1 == get_is_ata()))//zhaoqf_sh for auto test or AT cmd Test
    {
    ...
    else//zhaoqf_sh for manual test
    {
    pthread_create(&mc->update_thd, NULL, emmc_update_iv_thread, priv);
    do {
      chosen = iv->run(iv, &exit);
