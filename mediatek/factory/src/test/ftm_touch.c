...
//zhaoqf_sh add for full req
item_t ctype_touch_full_items[] = {
    item(ITEM_TOUCH_MAIN, uistr_info_touch_ctp_main),
    item(-1, NULL),
};
...
#define TOUCH_UPDATE_FR_DIV 5 // 2
#endif
...
static int m_ctp_enter_times = 0;
static bool m_ctp_main_ok = false;
static bool m_tp_free_ok = false;
...
static void clear_detect_date(void)
{
	...
	while(i < 50)
	{
		...
		detect_date.count_up1[i] = 0;
		detect_date.count_left1[i] = 0;
		detect_date.count_down1[i] = 0;
		detect_date.count_right1[i] = 0;
...
static void detect_points(struct touch *tpd, int x, int y)
{
	...
	int update_x, update_y;
        ...
        if(x <= 0 || y <= 0 || x >= tpd->width || y >= tpd->height){
            return;
        }
...
#ifndef ENTER_TP_TEST    
	if (TOUCH_L1*3 > y && TOUCH_L1*2 < y 
            && x >TOUCH_L1*2  && x <((tpd->width/TOUCH_L1-2) *TOUCH_L1)
            && g_pass_case1 == 0)       // up side
	{
		update_px = (x/TOUCH_L1) * TOUCH_L1;
		update_py = TOUCH_L1*2;
		update_x = (x/TOUCH_L1 + 1) * TOUCH_L1;
		update_y = TOUCH_L1*2;
		...
		if(x > tpd->width - TOUCH_L1)
			ui_line(tpd->width, 0, tpd->width, TOUCH_L1, 4);
		ui_color(255, 255, 255, 255);
		detect_date.count_up1[x/TOUCH_L1] = 1;
	}
#endif    
...
#ifndef ENTER_TP_TEST    
	if (x > ((tpd->width/TOUCH_L1-3) *TOUCH_L1) && x < ((tpd->width/TOUCH_L1-2) *TOUCH_L1)
            && y > TOUCH_L1*2 && y < (tpd->height/TOUCH_L1-2)*TOUCH_L1
            && g_pass_case1 == 0)       // right side
	{
		update_px = tpd->width -TOUCH_L1*2;
		update_py = (y/TOUCH_L1) * TOUCH_L1;
		update_x = tpd->width -TOUCH_L1*2;
		update_y = (y/TOUCH_L1 + 1) * TOUCH_L1;
		...
		ui_color(255, 255, 255, 255);
		detect_date.count_right1[y/TOUCH_L1] = 1;
	}
#endif
...
#ifndef ENTER_TP_TEST    
	if ((y > ((tpd->height/TOUCH_L1-3) *TOUCH_L1)) && y < ((tpd->height/TOUCH_L1-2) *TOUCH_L1)
                && x >TOUCH_L1*2  && x <(tpd->width/TOUCH_L1-2) *TOUCH_L1
                && g_pass_case1 == 0)       //down side
	{
		update_px = (x/TOUCH_L1) * TOUCH_L1;
		update_py = (tpd->height/TOUCH_L1-2)*TOUCH_L1;
		update_x = (x/TOUCH_L1 + 1) * TOUCH_L1;
		update_y = (tpd->height/TOUCH_L1-2)*TOUCH_L1;
		...
		ui_color(255, 255, 255, 255);
		detect_date.count_down1[x/TOUCH_L1] = 1;
	}
#endif
...
#ifndef ENTER_TP_TEST    
	if (x > TOUCH_L1*2 && x < TOUCH_L1*3
            && y > TOUCH_L1*2 && y < (tpd->height/TOUCH_L1-2)*TOUCH_L1
            && g_pass_case1 == 0)       //left side
	{
		update_px = TOUCH_L1*2;
		update_py = (y/TOUCH_L1) * TOUCH_L1;
		update_x = TOUCH_L1*2;
		update_y = (y/TOUCH_L1 + 1) * TOUCH_L1;
		...
		ui_color(255, 255, 255, 255);
		detect_date.count_left1[y/TOUCH_L1] = 1;
	}
#endif
...
#ifndef ENTER_TP_TEST    
		      ui_line(TOUCH_L1*2,TOUCH_L1*2,tpd->width -TOUCH_L1*2,TOUCH_L1*2, 2);
                   ui_line(tpd->width -TOUCH_L1*2,TOUCH_L1*2,tpd->width -TOUCH_L1*2,(tpd->height/TOUCH_L1-2)*TOUCH_L1, 2);
		      ui_line(tpd->width -TOUCH_L1*2,(tpd->height/TOUCH_L1-2)*TOUCH_L1,TOUCH_L1*2,(tpd->height/TOUCH_L1-2)*TOUCH_L1, 2);
		      ui_line(TOUCH_L1*2,(tpd->height/TOUCH_L1-2)*TOUCH_L1,TOUCH_L1*2,TOUCH_L1*2, 2);
		
			paint_short_line_test(tpd, TOUCH_L1*2, TOUCH_L1*2, (tpd->width/TOUCH_L1-2)*TOUCH_L1, TOUCH_L1*2, 2, 1);		
			paint_short_line_test(tpd, (tpd->width/TOUCH_L1-2)*TOUCH_L1, TOUCH_L1*2, (tpd->width/TOUCH_L1-2)*TOUCH_L1, (tpd->height/TOUCH_L1-2)*TOUCH_L1, 2, 2);		
			paint_short_line_test(tpd, TOUCH_L1*2, (tpd->height/TOUCH_L1-2)*TOUCH_L1, (tpd->width/TOUCH_L1-2)*TOUCH_L1, (tpd->height/TOUCH_L1-2)*TOUCH_L1, 2, 3);		
			paint_short_line_test(tpd, TOUCH_L1*2, TOUCH_L1*2, TOUCH_L1*2, (tpd->height/TOUCH_L1-2)*TOUCH_L1, 2, 4);		

		      ui_line(TOUCH_L1*3,TOUCH_L1*3,(tpd->width/TOUCH_L1-3)*TOUCH_L1,TOUCH_L1*3, 2);
                   ui_line((tpd->width/TOUCH_L1-3)*TOUCH_L1,TOUCH_L1*3,(tpd->width/TOUCH_L1-3)*TOUCH_L1,(tpd->height/TOUCH_L1-3)*TOUCH_L1, 2);
		      ui_line((tpd->width/TOUCH_L1-3)*TOUCH_L1,(tpd->height/TOUCH_L1-3)*TOUCH_L1,TOUCH_L1*3,(tpd->height/TOUCH_L1-3)*TOUCH_L1, 2);
		      ui_line(TOUCH_L1*3,(tpd->height/TOUCH_L1-3)*TOUCH_L1,TOUCH_L1*3,TOUCH_L1*3, 2);
#endif
		}
...
static void part(struct touch *tpd, int mode) {
	if((1 == g_pass_case2) && (1 == is_full_mode))
	{
		ui_color(255, 255, 255, 255);
		ui_text(10,CHAR_HEIGHT,uistr_info_pass);
	}
	else
	{
		if(tpd->scene == ITEM_TOUCH_MAIN)
		{
			m_ctp_enter_times++;
		}
	    ui_color(255, 255, 255, 255);
...
static void *touch_update_thread(void *priv) {
    ...
    while (1) {
		if(1 == is_full_mode)
		{
			if(ui_key_pressed(get_back_fail_key()) || ui_key_pressed(CUST_KEY_CONFIRM))
			{
				if(ui_key_pressed(get_back_fail_key()) && (ev_head_num != 0 || ev_tail_num != 0))
				{
				...
				else if (ui_key_pressed(CUST_KEY_CONFIRM) && (1 == g_pass_case2))
				{
    LOGD(TAG "ui_key_pressed(CUST_KEY_CONFIRM) \n");
...
#ifndef ENTER_TP_TEST    
					i = 2;
					while(i < ((tpd->width-TOUCH_L1*2)/TOUCH_L1))
					{
						if(detect_date.count_up1[i] != 1 || 
						detect_date.count_down1[i] != 1)
						{
							detect_flags = 0;
							break;
						}else
...
#ifndef ENTER_TP_TEST    
					i = 2;
					while(i < (tpd->height - TOUCH_L1*2)/TOUCH_L1)
					{
						if(detect_date.count_left1[i] != 1 || 
						detect_date.count_right1[i] != 1)
						{
							detect_flags = 0;
							break;
						}else
...
int touch_entry(struct ftm_param *param, void *priv) {
    ...
    if (0){//type == '0') {
		...
		if(1 == is_full_mode)
		{
			tpd->menu.set_items(&(tpd->menu), ctype_touch_full_items, 0);
		}
		else
		{
	...
	if(1 == is_full_mode)
	{
		...
		tpd->moveto = ITEM_TOUCH_MAIN;
#endif

    pthread_create(&(tpd->update_thd), NULL, touch_update_thread, priv);
	clear_touch_result();
    do {
    	LOGD(TAG "%s, tpd->scene = %d\n", __FUNCTION__, tpd->scene);
        switch(tpd->scene) {
        case ITEM_TOUCH_CHOOSER:
            ...
            case ITEM_TOUCH_FAIL:
                if (tpd->scene == ITEM_TOUCH_PASS) {
					if(m_ctp_main_ok && (m_ctp_enter_times > 1))
					{
                    	tpd->mod->test_result = FTM_TEST_PASS;
					}
					else
			...
			//zhaoqf_sh add for full test mode							
			case 100:
			case 101:
                if (tpd->scene == 100) {
                    tpd->mod->test_result = FTM_TEST_PASS;
                } else if (tpd->scene == 101) {
                    tpd->mod->test_result = FTM_TEST_FAIL;
                }
                exit = true;
				break;
