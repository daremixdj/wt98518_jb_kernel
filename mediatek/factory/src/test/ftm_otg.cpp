...
static void otg_update_info(struct otg *hds, char *info)
{
	...
	if (!strncmp(w_buf, r_buf, BUF_LEN)) { /*the same*/
		hds->avail = true;
		hds->status_plugIn = true;
    } else {
    	hds->avail = false;
    }

	if(hds->status_plugIn) {
		if(strncmp(w_buf, r_buf, BUF_LEN)) {
			hds->status_plugOut = true;
		}
	}
...
EXIT:
	...
	//zhaoqf_sh add for test result display
	if(hds->status_plugOut)
	{
		ptr += sprintf(ptr, uistr_info_result " : %s\n", "Pass");
	}
	else 
	{
...
static void *otg_update_iv_thread(void *priv)
{
	...
	hds->status_plugIn = false;
	hds->status_plugOut = false;
...
int otg_entry(struct ftm_param *param, void *priv)
{
    ...
    {
    iv->set_items(iv, otg_auto_items, 0);
    }
    else
    {
