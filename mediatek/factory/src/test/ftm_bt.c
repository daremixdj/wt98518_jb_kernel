...
bool bt_test_pass = false; 
extern bool BT_is_testing; 
extern bool BT_test_canceled;
...
static void *bt_update_thread(void *priv)
{
    ...
    while (1) {
        ...
        if((1 == is_full_mode) && bt_test_pass) break;//zhaoqf_sh
...
int bt_entry(struct ftm_param *param, void *priv)
{
    ...
    if(1 == is_full_mode)
    {
    BT_test_canceled = false;//init
    iv->set_items(iv, bf_items, 0);
    }
    else
    {
    iv->set_items(iv, bf_manual_items, 0);
    }
	...
	else
	{
		...
		else
		{
			...
			if(bt_test_pass && !BT_test_canceled)
			{
				bf->mod->test_result = FTM_TEST_PASS;
			}
			BT_is_testing = false;
...
int bt_init(void)
{
    ...
    if (!mod)
        return -ENOMEM;
    mod->test_type = BACKGROUND_TEST;//zhaoqf_sh
    mod->needbackRun = true;//zhaoqf_sh
    bf = mod_to_bf(mod);
    bf->mod  = mod;
