...
static int keys_key_handler(int key, void *priv)
{
	...
	for (i = keys->pstart; i < pend; i++) {
        LOGD(TAG "keys_key_handler_get_keycode[%d]_%d\n", i, keys_keymap[i].code);
...
static int keys_entry(struct ftm_param *param, void *priv)
{
	...
	if(1 == get_is_ata())//zhaoqf_sh for ata not test(menu back home 3keys)
	{
		keys->ptested = keys->pnum - hardKey;
		keys->untested = hardKey;
	}
