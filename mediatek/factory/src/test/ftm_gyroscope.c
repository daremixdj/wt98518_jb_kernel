...
static void *gyro_update_iv_thread(void *priv)
{
    ...
    while (1) {
	   ...
	   FLPLOGE("gyro_update_info() = (%s), %d\n", strerror(errno), err);
		...
		#if 1//zhaoqf_sh modify for test result
        snprintf(dat->info, sizeof(dat->info), "X: %+7.4f \nY: %+7.4f \nZ: %+7.4f \nResult:%s\n\n", 
			gyro->gyro_x, gyro->gyro_y, gyro->gyro_z, (gyro->resultcode == 0)?uistr_info_sensor_pass:uistr_info_sensor_fail);
		#else
        snprintf(dat->info, sizeof(dat->info), "X: %+7.4f \nY: %+7.4f \nZ: %+7.4f \n", 
...
int gyro_entry(struct ftm_param *param, void *priv)
{
	...
	if(is_full_mode) {
	iv->set_items(iv, gyro_auto_items, 0);
	}
	else {
