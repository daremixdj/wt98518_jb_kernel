...
static int timeout = 40;//60; zhaoqf_sh modify 60 to 40
static init_flag = 0;
...
static item_t gps_manual_items[] = {
    item(ITEM_PASS,   uistr_pass),
    item(ITEM_FAIL,   uistr_fail),
    item(-1, NULL),
};
...
bool gps_test_pass = false;
bool gps_test_over = false;
extern bool GPS_is_testing;
...
static void gps_update_info(struct gps_desc *gps, char *info)
{
    ...
    if(sockfd != C_INVALID_SOCKET)
    {
        ...
        else if(total_leng > 0)
        {
            ...
            {
            LOGD(TAG"Fix NO fix ZHAOQF_SH");
                if((1 == is_full_mode) && gps_test_over)
                {
                    ptr += sprintf(ptr, "%s: %d\n", uistr_info_gps_ttff, ttff);

					if(0 == g_gpsInfo.SV_cnt)
					{
						ptr += sprintf(ptr, "\n FAIL \n");
					}
					else
            ...
            LOGD(TAG "zhaoqf_sh get gpsInfo_SV_CNT %d\n", g_gpsInfo.SV_cnt);	
...
static void *gps_update_iv_thread(void *priv)
{
    ...
    memset(gps->info, '\n', INFO_SIZE);
    sprintf(gps->info, "%s\n", uistr_info_gps_init);
    if(0 == is_full_mode) iv->redraw(iv);//zhaoqf_sh
    sleep(1);
...
static bool fullmodeCloseFirst = false;
int gps_entry(struct ftm_param *param, void *priv)
{
    ...
    fullmodeCloseFirst = false;
    entry_cnt++;
    LOGD(TAG "gps_entry_coming_count is %d\n", entry_cnt);    
    if(entry_cnt > 1)
    {
        gps_test_over = true;
    }
    ...
    {
    iv->set_items(iv, gps_manual_items, 0);
    }
    iv->set_text(iv, &gps->text);
    
	iv->start_menu(iv,0);

    if((1 == is_full_mode) && (entry_cnt > 1))
    {
        fullmodeCloseFirst = true;
        iv->redraw(iv);
		gps_update_info(gps, gps->info);
        do {
            chosen = iv->run(iv, &exit);
            LOGD(TAG "GPS_auto_test_chosen = %d\n",chosen);
            switch (chosen) {
            case 0://pass
            case 1://fail
                if (chosen == 0) {
	...
	if(1 == get_is_ata())//zhaoqf_sh for AT cmd 
	{
	iv->redraw(iv);
    do{
    ...
    //gps->exit_thd = true;
		...
		do {
			...
			if (exit) {
                ...
                entry_cnt = 0;//baal modify for B247476 
...
int gps_init(void)
{
    ...
    //zhaoqf_sh modify for full test mode
    mod->test_type = BACKGROUND_TEST;
    mod->needbackRun = true;
    //zhaoqf_sh modify end
    gps  = mod_to_gps(mod);
