...
static item_t audio_items_auto_loopback[] = {
#ifdef MTK_DUAL_MIC_SUPPORT
    {ITEM_MIC1, uistr_info_audio_loopback_dualmic_mi1, 0, 0},
    {ITEM_MIC2, uistr_info_audio_loopback_dualmic_mi2, 0, 0},
#else
...
static item_t audio_items_auto_acoustic_loopback[] = {
#ifdef MTK_DUAL_MIC_SUPPORT
	{ITEM_DUALMIC_DMNR, uistr_info_audio_acoustic_loopback_DMNR, 0, 0},
#endif
	{ITEM_DUALMIC_NO_DMNR, uistr_info_audio_acoustic_loopback_without_DMNR, 0, 0},
...
//zhaoqf_sh add for ex ringtone wav
static char * RingToneWavePlaybackEx = "/res/sound/ringtone_ex.wav";
    ...
    while (1)
    {
        ...
        while (((pFileList && !feof(pFileList) && FileListEnd == false) || (WavePlayInstance.ThreadStart == true)) && hds->exit_thd == false)
        {
            ...
            if (WavePlayInstance.ThreadStart == false)
            {
                ...
                // crop file name to waveplay data structure
                if((hds->ringtoneEx_exist) && (0 == get_is_ata()))
                {
					filelength = strlen(RingToneWavePlaybackEx);
					if(filelength >0){
						WavePlayInstance.FileName= new char[filelength+1];
						memset((void*)WavePlayInstance.FileName,'\0',filelength+1);
						memcpy((void*)WavePlayInstance.FileName,(void*)RingToneWavePlaybackEx,filelength);
					}
				}
				else
				{
...
int mAudio_loopback_phonemicspk_entry(struct ftm_param *param, void *priv)
{
	...
	mc->i4Playtime = 2*1000;//ms  // mtk zyt for ata get_is_ata
    pthread_create(&mc->hHeadsetThread, NULL, Audio_Wave_Playabck_thread, priv);
    pthread_create(&mc->hRecordThread, NULL, Audio_Record_thread, priv);

	if(1 == get_is_ata())//zhaoqf_sh
	{
	...
	else
	{
		do {
			...
			if (exit) {
				LOGD("mAudio_loopback_phonemicspk_entry set exit_thd = true\n");
				mc->exit_thd = true;
				break;
			}
		} while (1);
...
int mAudio_receiver_entry(struct ftm_param *param, void *priv)
{
	...
	FILE *pFileListEx = NULL;
	pFileListEx = fopen(RingToneWavePlaybackEx,"rb");
	if(pFileListEx == NULL)
	{
		mc->ringtoneEx_exist = false;
	}
	else
	{
		mc->ringtoneEx_exist = true;
	}
    ...
    iv = mc->iv;
    iv->set_title(iv, &mc->title);
    if(1 == is_full_mode)//zhaoqf_sh for auto test
    {
    iv->set_items(iv, audio_items_auto, 0);
    }
    else
	...
	if(1 == get_is_ata())//zhaoqf_sh delay ringtone time
	{
	//mc->i4Playtime = read_preferred_ringtone_time()*1000;//ms
	mc->i4Playtime = 3*1000;//ms  // mtk zyt
	}
	else
	{
	mc->i4Playtime = 2*1000;
	}
...
int mAudio_reveiverloopback_entry(struct ftm_param *param, void *priv)
{
    ...
    {
	auto_select = true;
    iv->set_items(iv, audio_items_auto_loopback, 0);    
    }
    else
    {
...
#if 0//def FEATURE_FTM_HEADSET//zhaoqf_sh modify
    ...
    do
    {
		...
		case 100://pass
		case 101://fail
            if ((chosen == ITEM_PASS) || (chosen == 100))//zhaoqf_sh
                {
            ...
            else if ((chosen == ITEM_FAIL) || (chosen == 101))//zhaoqf_sh
                {
                    mc->mod->test_result = FTM_TEST_FAIL;
                }
	...
	if(1 == is_full_mode)//zhaoqf_sh add for full test mode
	{		
		auto_select = false;
	}
...
int mAudio_Acoustic_Loopback_entry(struct ftm_param *param, void *priv)
{
    ...
    {
    auto_select = true;
    iv->set_items(iv, audio_items_auto_acoustic_loopback, 0);
    }
    else
    {
...
int mAudio_waveplayback_entry(struct ftm_param *param, void *priv)
{
	...
	{
#ifdef FEATURE_FTM_WAVE_RING_DIFF_WITH_RECEIVE
	mc->ringtoneEx_exist = false;
    strcpy(mc->file_name,FileStartString_WavePlayback);
#else
    strcpy(mc->file_name,FileStartString_LoudspkPlayback);
...
EXIT:
    ...
    /* preare text view info */
#if 1//zhaoqf_sh modify 
	int len1=0, len2=0;
	ptr = info;
	if(!hds->avail)
	{
		len1 = sprintf(ptr, uistr_info_audio_headset_note);
	}
	ptr += len1;
	len2 = sprintf(ptr, uistr_info_audio_headset_avail, hds->avail ? uistr_info_audio_yes : uistr_info_audio_no);
	ptr += len2;
#else
	ptr = info;
...
int Audio_PhoneMic1_Speaker_loopback_entry(struct ftm_param *param, void *priv)
{
    ALOGD("Select Audio_PhoneMic1_Speaker_loopback_entry");
    int chosen;
    bool exit = false;
    struct mAudio *mc = (struct mAudio *)priv;
    mc->exit_thd = false;
...
int Audio_PhoneMic2_Speaker_loopback_entry(struct ftm_param *param, void *priv)
{
    ALOGD("Select Audio_PhoneMic2_Speaker_loopback_entry");
    int chosen;
    bool exit = false;
    struct mAudio *mc = (struct mAudio *)priv;
    mc->exit_thd = false;
...
static void *audio_phoneMicToHeadset_loopback_open(void *mPtr)
{
   struct mAudio *hds = (struct mAudio *)mPtr;
   struct itemview *iv = hds->iv;
   ...
   ::ioctl(HeadsetFd,SET_CALL_STATE,1);
	...
	Audio_headset_info(hds, hds->info);	
	PhoneMic_EarphoneLR_Loopback(MIC1_ON);
...
int audio_phoneMicToHeadset_loopback_close(void)
{
	// set headset state to 0 , lose headset device.
	::ioctl(HeadsetFd,SET_CALL_STATE,0);
...
int Audio_PhoneMicToHeadset_loopback_entry(struct ftm_param *param, void *priv)
{
    struct mAudio*mc = (struct mAudio *)priv;
    mc->exit_thd = false;
    mc->Headset_change = false;
	...
	Audio_headset_info(mc, mc->info);

	if(get_is_ata())
	{
		int write_len = write(usb_com_port, "pass\r\n", strlen("pass\r\n"));
		if(write_len != strlen("pass\r\n"))
		{
			LOGD(TAG "PhoneMIC_to_headset write data to pc fail\n");
		}
	}
	audio_phoneMicToHeadset_loopback_open(priv);
...
static void *audio_phoneMic2ToHeadset_loopback_open(void *mPtr)
{
   struct mAudio *hds = (struct mAudio *)mPtr;
   struct itemview *iv = hds->iv;
...
int audio_phoneMic2ToHeadset_loopback_close(void)
{
	// set headset state to 0 , lose headset device.
	::ioctl(HeadsetFd,SET_CALL_STATE,0);
...
int Audio_PhoneMic2ToHeadset_loopback_entry(struct ftm_param *param, void *priv)
{
    struct mAudio*mc = (struct mAudio *)priv;
    mc->exit_thd = false;
    mc->Headset_change = false;
	...
	if(get_is_ata())
	{
		...
		if(write_len != strlen("pass\r\n"))
		{
			LOGD(TAG "PhoneMIC2_to_headset write data to pc fail\n");
		}
	}
	audio_phoneMic2ToHeadset_loopback_open(priv);
...
int audio_HMicToReveiver_loopback_close(void)
{
	// set headset state to 0 , lose headset device.
	::ioctl(HeadsetFd, SET_CALL_STATE, 0);
	...
	HeadsetMic_Receiver_Loopback(MIC1_OFF, NULL);
...
static void *audio_HMicToReveiver_loopback_open(void *mPtr)
{
    struct mAudio *hds = (struct mAudio *)mPtr;
    struct itemview *iv = hds->iv;
    ...
    if (hds->avail)
    {
        HeadsetMic_Receiver_Loopback(MIC1_ON, NULL);
    }
...
int Audio_HeadsetMic_Receiver_loopback_entry(struct ftm_param *param, void *priv)
{
	struct mAudio *mc = (struct mAudio *)priv;
	mc->exit_thd = false;
	mc->Headset_change = false;
	...
	if(get_is_ata())
	{
		...
		if(write_len != strlen("pass\r\n"))
		{
			LOGD(TAG "headsetMIC_to_receiver write data to pc fail\n");
		}
	}

	audio_HMicToReveiver_loopback_open(priv);
...
int audio_HeadsetToSPK_loopback_close(void)
{
	// set headset state to 0 , lose headset device.
	::ioctl(HeadsetFd, SET_CALL_STATE, 0);
...
static void *audio_HeadsetToSPK_loopback_open(void *mPtr)
{
    struct mAudio *hds = (struct mAudio *)mPtr;
    struct itemview *iv = hds->iv;
    ALOGD(TAG "%s: Start\n", __FUNCTION__);
...
int Audio_HeadsetMic_SPK_loopback_entry(struct ftm_param *param, void *priv)
{
    struct mAudio *mc = (struct mAudio *)priv;
    mc->exit_thd = false;
    mc->Headset_change = false;
	...
	if(get_is_ata())
	{
		...
		if(write_len != strlen("pass\r\n"))
		{
			LOGD(TAG "headsetMIC_to_speaker write data to pc fail\n");
		}
	}

	audio_HeadsetToSPK_loopback_open(priv);
...
int audio_HMicToEar_loopback_close(void)
{
	// set headset state to 0 , lose headset device.
	::ioctl(HeadsetFd, SET_CALL_STATE, 0);
	...
	HeadsetMic_EarphoneLR_Loopback(MIC1_OFF, NULL);
	g_loopback_item = 0;
	Common_Audio_deinit();
...
static void *audio_HMicToEar_loopback_open(void *mPtr)
{
    struct mAudio *hds = (struct mAudio *)mPtr;
    struct itemview *iv = hds->iv;
    ...
    if (hds->avail)
    {
        HeadsetMic_EarphoneLR_Loopback(MIC1_ON, NULL);
    }
...
int Audio_HeadsetMic_EarPhone_loopback_entry(struct ftm_param *param, void *priv)
{
	struct mAudio *mc = (struct mAudio *)priv;
	mc->exit_thd = false;
	mc->Headset_change = false;
	...
	if(get_is_ata())
	{
		...
		if(write_len != strlen("pass\r\n"))
		{
			LOGD(TAG "headsetMIC_to_earPhone write data to pc fail\n");
		}
	}
	
	audio_HMicToEar_loopback_open(priv);
...
int audio_init(void)
{
    ...
    struct ftm_module *modLoudspk, *modReceiver,
				...
				*modPMic2_Headset_Loopback,
				*modPMic_SPK_Loopback,
				*modPMic1_SPK_Loopback,
				*modPMic2_SPK_Loopback,
				*modHMic_SPK_Loopback,
				*modHMic_RECV_Loopback,
				*modHMic_SPEAKER_Loopback,
				*modHMic_EAR_Loopback,
				*modWavePlayback,
    ...
    struct mAudio *maudioSpk, *maudioReveiver,
				...
				*maudioPMic2_Headset_Loopback,
				*maudioPMic_SPK_Loopback,
				*maudioPMic1_SPK_Loopback,
				*maudioPMic2_SPK_Loopback,
				*maudioHMic_SPK_Loopback,
				*maudioHMic_RECV_Loopback,
				*maudioHMic_SPEAKER_Loopback,
				*maudioHMic_EAR_Loopback,
				*maudioWavePlayback,
    ...
    if (!modLoudspk)
{
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
    modPMic1_SPK_Loopback = ftm_alloc(ITEM_LOOPBACK2_MIC1, sizeof(struct mAudio));
    maudioPMic1_SPK_Loopback = mod_to_mAudio(modPMic1_SPK_Loopback);
    maudioPMic1_SPK_Loopback->mod = modPMic1_SPK_Loopback;
    if (!modPMic1_SPK_Loopback)
    {
        return -ENOMEM;
    }
    ret = ftm_register(modPMic1_SPK_Loopback, Audio_PhoneMic1_Speaker_loopback_entry, (void *)maudioPMic1_SPK_Loopback);

    modPMic2_SPK_Loopback = ftm_alloc(ITEM_LOOPBACK2_MIC2, sizeof(struct mAudio));
    maudioPMic2_SPK_Loopback = mod_to_mAudio(modPMic2_SPK_Loopback);
    maudioPMic2_SPK_Loopback->mod = modPMic2_SPK_Loopback;
    if (!modPMic2_SPK_Loopback)
    {
        return -ENOMEM;
    }
    ret = ftm_register(modPMic2_SPK_Loopback, Audio_PhoneMic2_Speaker_loopback_entry, (void *)maudioPMic2_SPK_Loopback);
#endif
...
#ifdef WT_FTM_PHONE_MIC_HEADSET_LOOPBACK
	modPMic_Headset_Loopback = ftm_alloc(ITEM_LOOPBACK_PMIC_HEADSET, sizeof(struct mAudio));
	maudioPMic_Headset_Loopback = mod_to_mAudio(modPMic_Headset_Loopback);
	maudioPMic_Headset_Loopback->mod = modPMic_Headset_Loopback;
	if (!modPMic_Headset_Loopback)
	{
	...
	ret = ftm_register(modPMic_Headset_Loopback, Audio_PhoneMicToHeadset_loopback_entry, (void*)maudioPMic_Headset_Loopback);
#endif
	
#ifdef WT_FTM_PHONE_REFMIC_HEADSET_LOOPBACK
	modPMic2_Headset_Loopback = ftm_alloc(ITEM_LOOPBACK_PMIC2_HEADSET, sizeof(struct mAudio));
	maudioPMic2_Headset_Loopback = mod_to_mAudio(modPMic2_Headset_Loopback);
	maudioPMic2_Headset_Loopback->mod = modPMic2_Headset_Loopback;
	if (!modPMic2_Headset_Loopback)
	{
		return -ENOMEM;
	}
	ret = ftm_register(modPMic2_Headset_Loopback, Audio_PhoneMic2ToHeadset_loopback_entry, (void*)maudioPMic2_Headset_Loopback);
#endif
	
#ifdef WT_FTM_HEADSET_MIC_RECEIVER_LOOPBACK
	modHMic_RECV_Loopback = ftm_alloc(ITEM_LOOPBACK_HMIC_RECEIVER, sizeof(struct mAudio));
	maudioHMic_RECV_Loopback = mod_to_mAudio(modHMic_RECV_Loopback);
	maudioHMic_RECV_Loopback->mod = modHMic_RECV_Loopback;
	if (!modHMic_RECV_Loopback)
	{
		return -ENOMEM;
	}
	ret = ftm_register(modHMic_RECV_Loopback, Audio_HeadsetMic_Receiver_loopback_entry, (void *)maudioHMic_RECV_Loopback);
#endif
		
#ifdef WT_FTM_HEADSET_MIC_SPEAKER_LOOPBACK
	modHMic_SPEAKER_Loopback = ftm_alloc(ITEM_LOOPBACK_HMIC_SPK, sizeof(struct mAudio));
	maudioHMic_SPEAKER_Loopback = mod_to_mAudio(modHMic_SPEAKER_Loopback);
	maudioHMic_SPEAKER_Loopback->mod = modHMic_SPEAKER_Loopback;
	if (!modHMic_SPEAKER_Loopback)
	{
		return -ENOMEM;
	}
	ret = ftm_register(modHMic_SPEAKER_Loopback, Audio_HeadsetMic_SPK_loopback_entry, (void *)maudioHMic_SPEAKER_Loopback);
#endif
		
#ifdef WT_FTM_HEADSET_MIC_EARPHONE_LOOPBACK
	modHMic_EAR_Loopback = ftm_alloc(ITEM_LOOPBACK_HMIC_EARPHONE, sizeof(struct mAudio));
	maudioHMic_EAR_Loopback = mod_to_mAudio(modHMic_EAR_Loopback);
	maudioHMic_EAR_Loopback->mod = modHMic_EAR_Loopback;
	if (!modHMic_EAR_Loopback)
	{
		return -ENOMEM;
	}
	ret = ftm_register(modHMic_EAR_Loopback, Audio_HeadsetMic_EarPhone_loopback_entry, (void *)maudioHMic_EAR_Loopback);
#endif
