...
//zhaoqf_sh add for Gsensor_test_range
#if 1//def GSENSOR_RANGE_1
#define TOLERANCE_1G_L            (GRAVITY_EARTH-1)
#define TOLERANCE_1G_U            (GRAVITY_EARTH+1)
#else
#define TOLERANCE_1G_L            (8)
#define TOLERANCE_1G_U            (12)
#endif

//#ifdef FEATURE_45_DEGREE_TEST
#define GRAVITY_45_DEGREE_X (-5.0)
#define GRAVITY_45_DEGREE_Y (5.5)
#define GRAVITY_45_DEGREE_Z (6.7)

#define TOLERANCE_45_DEGREE  1
//#endif

static bool Z_test_ok =false;
static bool alltestok = false;
static bool curstatus = false;
/******************************************************************************
 * Structure
 *****************************************************************************/
enum {
...
static item_t gsensor_auto_items[] = {
    item(-1, NULL),
};
...
{
#ifdef FEATURE_45_DEGREE_TEST
    if ((acc->evt.x  >  GRAVITY_45_DEGREE_X-TOLERANCE_45_DEGREE) && (acc->evt.x  <  GRAVITY_45_DEGREE_X+TOLERANCE_45_DEGREE) &&//(acc->evt.x  >  TOLERANCE_1G) && 
        (acc->evt.y  > GRAVITY_45_DEGREE_Y-TOLERANCE_45_DEGREE  &&  acc->evt.y  <  GRAVITY_45_DEGREE_Y+TOLERANCE_45_DEGREE) &&
        (acc->evt.z  > -GRAVITY_45_DEGREE_Z-TOLERANCE_45_DEGREE  &&  acc->evt.z  <  GRAVITY_45_DEGREE_Z+TOLERANCE_45_DEGREE)) {
        acc->tilt = TILT_45_DEGREE;
        return 0;
    }
#endif
    if ((acc->evt.x  >  TOLERANCE_1G_L) && (acc->evt.x  <  TOLERANCE_1G_U) &&//(acc->evt.x  >  TOLERANCE_1G) && 
    ...
    if ((acc->evt.y  >  TOLERANCE_1G_L) && (acc->evt.y  <  TOLERANCE_1G_U) &&//(acc->evt.y  >  TOLERANCE_1G) && 
    ...
    if ((acc->evt.z  >  TOLERANCE_1G_L) && (acc->evt.z  <  TOLERANCE_1G_U) &&//(acc->evt.z  >  TOLERANCE_1G) && 
        (acc->evt.x  > -TOLERANCE_0G  &&  acc->evt.x  <  TOLERANCE_0G) &&
        (acc->evt.y  > -TOLERANCE_0G  &&  acc->evt.y  <  TOLERANCE_0G)) {
        acc->tilt = TILT_Z_POS;
        return 0;
...
static void *gsensor_update_iv_thread(void *priv)
{
    ...
    while (1) {
		...
		if(0 == is_full_mode)//zhaoqf_sh for manul test
		{
        ...
        len = snprintf(dat->info+len, sizeof(dat->info)-len, "%+6.3f %+6.3f %+6.3f\n%s (%s)\n%s: %s %s: %s\n%s: %s %s: %s\n%s: %s %s: %s\n%s: %s\n", 
                 ...
                 gsensor_pos[TILT_45_DEGREE], (acc->pos_chk & (1 << TILT_45_DEGREE)) ? (uistr_info_sensor_pass) : (uistr_info_g_sensor_testing));
#else
        len = snprintf(dat->info+len, sizeof(dat->info)-len, "%+6.3f %+6.3f %+6.3f\n%s (%s)\n%s: %s %s: %s\n%s: %s %s: %s\n%s: %s %s: %s\n", 
                 acc->evt.x, acc->evt.y, acc->evt.z, 
                 (acc->tilt != TILT_UNKNOWN) ? (uistr_info_sensor_pass) : ("NG"), gsensor_pos[acc->tilt],
                ...
                if(acc->tilt != TILT_UNKNOWN){
                    curstatus = true;
                }else{
                    curstatus = false;
                }
		   LOGE(TAG "pos_chk=%d, curstatus=%d\n", acc->pos_chk, curstatus); 
            
                if((acc->pos_chk & 0xfe) >= 2/*== 0x7e*/){// >=2 one pass is ok, ==7e all 6 direct pass
                    alltestok = true;
                }
		if(len < 0)
		{
		...
		else//zhaoqf_sh for auto test
		{
			len = 0;
#ifdef FEATURE_45_DEGREE_TEST
                    if(acc->pos_chk & (1 << TILT_45_DEGREE))
                    {
                        Z_test_ok = true;
                    }
                    len = snprintf(dat->info+len, sizeof(dat->info)-len, "%+6.3f %+6.3f %+6.3f\n%s(%s)\n\n%s: %s\n", 
                            acc->evt.x, acc->evt.y, acc->evt.z, 
                             (acc->tilt == TILT_45_DEGREE) ? (uistr_info_sensor_pass) : ("NG"), gsensor_pos[TILT_45_DEGREE],
                             gsensor_pos[TILT_45_DEGREE], (acc->pos_chk & (1 << TILT_45_DEGREE)) ? (uistr_info_sensor_pass) : (uistr_info_g_sensor_testing)
                            );
                    if(acc->tilt == TILT_45_DEGREE){
                        curstatus = true;
                    }else{
                        curstatus = false;
                    }
#else
			if((acc->pos_chk & (1 << TILT_Z_POS)) || (acc->pos_chk & (1 << TILT_Z_NEG)))
			{
				Z_test_ok = true;
			}
			len = snprintf(dat->info+len, sizeof(dat->info)-len, "%+6.3f %+6.3f %+6.3f\n%s: %s %s: %s\n\n\n%s\n", 
					 ...
					 gsensor_pos[TILT_Z_POS], (acc->pos_chk & (1 << TILT_Z_POS)) ? (uistr_info_sensor_pass) : (uistr_info_g_sensor_testing),
					 gsensor_pos[TILT_Z_NEG], (acc->pos_chk & (1 << TILT_Z_NEG)) ? (uistr_info_sensor_pass) : (uistr_info_g_sensor_testing),
					 uistr_gsensor_hint_info);
                    if(acc->tilt == TILT_Z_POS || acc->tilt == TILT_Z_NEG ){
                        curstatus = true;
                    }else{
                        curstatus = false;
                    }
...
int gsensor_entry(struct ftm_param *param, void *priv)
{
    ...
    {
    iv->set_items(iv, gsensor_auto_items, 0);
    }
    else
    {
    ...
    do {
        if(!support_ata){
        chosen = iv->run(iv, NULL/*&thread_exit*/);
        LOGD(TAG "%s chosen=%d, thread_exit=%d\n", __FUNCTION__, chosen, thread_exit);
        switch (chosen) {
        case ITEM_PASS:
        case ITEM_FAIL:
            if (chosen == ITEM_PASS) {
	...
	Z_test_ok = false; //zhaoqf_sh add for full test new mode
