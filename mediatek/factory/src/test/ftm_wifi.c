...
static item_t wifi_mannal_items[] = {
    item(ITEM_PASS,   uistr_info_wifi_test_pass),
    item(ITEM_FAIL,   uistr_info_wifi_test_fail),
    item(ITEM_RENEW,  uistr_info_wifi_renew),
    item(-1, NULL),
...
bool wifi_test_pass = false; 
extern bool wifi_test_canceled;
/*zhaoqf_sh add for full test new mode   end*/
static bool m_wifi_test_pass = false;
...
static void *wifi_update_thread(void *priv)
{
    ...
    if(0 == is_full_mode) iv->redraw(iv); //zhaoqf_sh add for full test new mode
    ...
    while (1) {
        ...
        if(1 == is_full_mode) break;//zhaoqf_sh add for full test new mode
...
extern bool wifi_is_testing; //zhaoqf_sh add for full test new mode
...
int wifi_entry(struct ftm_param *param, void *priv)
{
    ...
    if((1 == is_full_mode) || (1 == get_is_ata()))//zhaoqf_sh
    {
    wifi_test_canceled = false;//init
    iv->set_items(iv, wifi_items, 0);
    }
    else
    {
    iv->set_items(iv, wifi_mannal_items, 0);
    }
    iv->set_text(iv, &wififm->text);
	iv->start_menu(iv,0);
	
    if(0 == is_full_mode) iv->redraw(iv);//zhaoqf_sh for full test mode
    LOGD(TAG "%s new2\n", __FUNCTION__);
/*
    tv = &wififm->tv;
...
*/
	...
	else
	{
		...
		wififm->result = false;
		wififm->renew = true;  
		wififm->mod->test_result = FTM_TEST_FAIL; //zhaoqf_sh	 
	...
	m_wifi_test_pass = false;
		...
		{
			wifi_is_testing = true;
			wififm->mod->test_result = FTM_TEST_FAIL;
			pthread_join(wififm->update_thd, NULL);    
			if(wifi_test_pass && !wifi_test_canceled)
				wififm->mod->test_result = FTM_TEST_PASS;
