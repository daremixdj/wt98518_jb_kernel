...
int led_entry(struct ftm_param *param, void *priv)
{
    ...
    {
    #if defined(CUST_HAVE_NLEDS) && defined(CUST_HAVE_KEYPAD_LED)
    auto_select = true;
	#endif
	iv->set_items(iv, auto_items, 0);
    }
    else
    {
	...
	if(1 != get_is_ata())
	{
#ifdef CUST_HAVE_NLEDS//zhaoqf_sh add for Led Auto_test
	keypadled_cnt = 0;	
	if(nled_cnt == 0)
	{
	...
	nled_cnt++;
#elif defined(CUST_HAVE_KEYPAD_LED)//not define CUST_HAVE_NLEDS
	nled_cnt = 0;
	if(keypadled_cnt == 0)
	{
		if(led_thread_start)
