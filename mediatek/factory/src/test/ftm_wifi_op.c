...
extern bool wifi_test_pass;  //zhaoqf_sh add for full test new mode
...
/*zhaoqf_sh add for full test new mode   start*/	
		if(is_full_mode){
			...
			if(0!=item->ap_addr.sa_data[0] || 0!=item->ap_addr.sa_data[1] || 0!=item->ap_addr.sa_data[2]){
				wifi_test_pass = true;
				goto finish_test;
			}
			if(wifi_test_pass) goto finish_test;
		}
...
finish_test:    //zhaoqf_sh add for full test new mode    
    if( scanlist.result )
...
#if 1//zhaoqf_sh modify for ATA test
	temp = "mtkguest";
#else
