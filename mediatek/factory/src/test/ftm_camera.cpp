...
static item_t camera_auto_items[] = {
    {ITEM_PASS, " ", 0},    
    {-1, NULL,   0},
};
...
static int camera_cap_key_handler(int key, void *priv)
{
    ...
    case UI_KEY_CONFIRM:
        FTM_CAMERA_DBG("Key_Confirm Click - Capture \n");
            ...
            handled = -1;
#else
        if (gcamera_autoTest == 1 || 1 == get_is_ata())
...
static void *camera_update_showImg_tv_thread(void *priv)
{
    ...
    init_text(&cam ->right_btn, "", COLOR_YELLOW);
...
//    PFLASH_LIGHT_FUNCTION_STRUCT pstFlash;
    ...
    if(1 == is_full_mode)//zhaoqf_sh
    {
    iv->set_items(iv, camera_auto_items, 0);
    }
    else
    {
...
extern "C" int camera_lowlevel_init(void)
{
