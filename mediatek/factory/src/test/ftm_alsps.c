...
static item_t alsps_auto_items[] = {
    item(-1, NULL),
};
...
static bool sALSpass = false;
static bool sPSpass = false;
...
{
    int err = -EINVAL;
    int als_dat, ps_dat, ps_threshold_dat, ps_high, ps_low, ps_value;//yucong add for factory mode test support
    if (lps->fd == -1) {
        FLPLOGE("invalid fd\n");
        return -EINVAL;
        ...
        return err;
    }else if ((err = ioctl(lps->fd, ALSPS_GET_PS_DATA, &ps_value))) {
        FLPLOGE("read ps value: %d(%s)\n", errno, strerror(errno));
        return err;
...
static void *alsps_update_iv_thread(void *priv)
{
    ...
    int err = 0, len = 0;
	int ps_time = 0 , tmp = 0, ps_srn_data = 0 , total_time = 300;
	int als_tmp = 0 , ps_tmp = 0, als_test = 0, ps_test = 0, ps_value_close_tmp = 1, ps_value_far_tmp = 0;
    char *status;
	...
	//-----------------------------
		...
		if((ps_time >=10)&&(ps_time < 20)){
			  	tmp += lps->ps_raw;
			}
		if(ps_time == 20){
			ps_srn_data = tmp / 10;
			if((1 == lps->ps_value)||(0 == lps->ps_value)){
					ps_value_close_tmp = lps->ps_value;
				}
			}
		if(ps_time == 10){
			als_tmp = lps->als_raw;
	...
	//-----------------------------
        ...
        len = 0;
		len += snprintf(dat->info+len, sizeof(dat->info)-len, "ALS: %d  (0:dark; +:bright) \n", lps->als_raw);  
		if(lps->ps_value == 0){
        len += snprintf(dat->info+len, sizeof(dat->info)-len, "PS : %d   Close \n\n", lps->ps_raw);
		}else if(lps->ps_value == 1){
		len += snprintf(dat->info+len, sizeof(dat->info)-len, "PS : %d   Far \n\n", lps->ps_raw);
		}

	//-----------------------------
	if(ps_time > 10){
			if(als_tmp != lps->als_raw)
			{
				als_test = 1;
			}
			
			if(ps_tmp != lps->ps_raw)
			{
				ps_test = 1;
			}
			...
			if(als_test == 1)
			{
				len += snprintf(dat->info+len, sizeof(dat->info)-len, "ALS: Pass\n\n");
                            sALSpass = true;
			}
		}

	if(ps_time >= 20){
			if(ps_srn_data < PS_SRN_RAW_DATA){
			//------------ close ------------
				if(ps_time < total_time){
					ps_time++;
					len += snprintf(dat->info+len, sizeof(dat->info)-len, "SRN=%d \n\n%s %d\n\n",ps_srn_data,uistr_ps_sensor_test,((total_time-ps_time)/10));
				}
				if((ps_time > 20)&&(ps_time<total_time)&& !((1 == lps->ps_value)||(0 == lps->ps_value))){
					ps_time = total_time;
				}
				if((ps_time < total_time)&&(ps_value_close_tmp != lps->ps_value)&&((1 == lps->ps_value)||(0 == lps->ps_value))){
					ps_time = total_time+10;
				}
				if(ps_time == total_time){
					len += snprintf(dat->info+len, sizeof(dat->info)-len, "SRN=%d, PS: FAIL\n\n",ps_srn_data);
				}
				if(ps_time == (total_time+10)){
					ps_time = 2*total_time;
					ps_value_far_tmp = lps->ps_value;
				}
			//------------ close -------------
			//------------ far_away ----------
				if((ps_time >= (2*total_time))&&(ps_time < (3*total_time))){
					ps_time++;
					len += snprintf(dat->info+len, sizeof(dat->info)-len, "SRN=%d \n\n%s  %d\n\n",ps_srn_data,uistr_ps_far_sensor_test,(((3*total_time)-ps_time)/10));
				}
				if((ps_time >= (2*total_time))&&(ps_time < (3*total_time))&&(ps_value_far_tmp != lps->ps_value)&&((1 == lps->ps_value)||(0 == lps->ps_value))){
					ps_time = (3*total_time+10);
				}
				if(ps_time == (3*total_time)){
					len += snprintf(dat->info+len, sizeof(dat->info)-len, "SRN=%d, PS: FAIL\n\n",ps_srn_data);
				}
				if(ps_time == (3*total_time+10)){
					len += snprintf(dat->info+len, sizeof(dat->info)-len, "SRN=%d, PS: PASS\n\n",ps_srn_data);
                                sPSpass = true;
				}
			//------------ far_away ----------				
			}else if(ps_srn_data >= PS_SRN_RAW_DATA){
				len += snprintf(dat->info+len, sizeof(dat->info)-len, "SRN=%d, PS is out of rang: FAIL\n",ps_srn_data);
			}
		}			
	//-----------------------------
	...
	len += snprintf(dat->info+len, sizeof(dat->info)-len, "%s: %s\n", uistr_info_sensor_alsps_result, (lps->ps_threshold_value == 1)?uistr_info_sensor_pass:uistr_info_sensor_fail	);//yucong add for factory mode test support
        ...
        if(1 == is_full_mode && sALSpass == true && sPSpass == true){
            LOGD(TAG "%s: fullmode test ok!!!\n", __FUNCTION__);
            thread_exit = true;  
...
int alsps_entry(struct ftm_param *param, void *priv)
{
    ...
    {
    iv->set_items(iv, alsps_auto_items, 0);
    }
    else
    {
    ...
    do {
        if(!support_ata){
            if(1 == is_full_mode){
                if(ui_key_pressed(get_back_fail_key())){
