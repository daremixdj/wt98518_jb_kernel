...
static item_t sim_items[] = {
  item(ITEM_SIM1, uistr_info_sim_detect_item_sim_1),
#ifdef GEMINI
  item(ITEM_SIM2, uistr_info_sim_detect_item_sim_2),
#endif
...
#define SIM_TEST_PASS	1
#define SIM_TEST_FAIL	2
int sim_test_result[3] = {0};
/*zhaoqf_sh add for full test new mode   end*/
bool sim_test_pass = false;
    ...
    while(1) {
        ...
        if (!sim->test_done) {
            ...
            if(ret == RET_ESIMS_YES) {
				...
				sim_test_result[sim->sim_id] = SIM_TEST_PASS;//zhaoqf_sh
            } else {
                sprintf(sim->info + strlen(sim->info),
                    "%s%d: %s!!\n", uistr_info_detect_sim, sim->sim_id, uistr_info_fail);
                LOGD(TAG "Detect SIM%d: Fail.\n",sim->sim_id);
				sim_test_result[sim->sim_id] = SIM_TEST_FAIL;//zhaoqf_sh
            }
            //LOGD(TAG "redraw\n");
...
int sim_entry(struct ftm_param *param, void *priv)
{
	...
	if(1 == is_full_mode)
	{
	iv->set_items(iv, sim_auto_items, 0);
	}
	else
	{
    ...
    if(1 == is_full_mode)
    {
        ...
        sim->sim_id = SIM_ID_1;
        sim->test_done = false;
        while(0==sim_test_result[SIM_ID_1]) usleep(200000);
#ifdef GEMINI
        sim->sim_id = SIM_ID_2;
        sim->test_done = false;
        while(0==sim_test_result[SIM_ID_2]) usleep(200000);
        if((SIM_TEST_PASS==sim_test_result[SIM_ID_1]) && (SIM_TEST_PASS==sim_test_result[SIM_ID_2]))
        {
            sim_test_pass = true;
        }
#else
        if(SIM_TEST_PASS==sim_test_result[SIM_ID_1])
        {
            sim_test_pass = true;
        }
#endif
...
int sim_init(void)
{
	...
	//zhaoqf_sh for full test mode start
