...
static item_t mcard_manual_items[] = {
    item(ITEM_PASS,   uistr_pass),
    item(ITEM_FAIL,   uistr_fail),
    item(-1, NULL),
};
...
int g_test_result = 0;
static bool support_ata = false;//zhaoqf_sh for AT cmd
...
int mcard_entry(struct ftm_param *param, void *priv)
{
    ...
    else//for manual test
    {
    iv->set_items(iv, mcard_manual_items, 0);
    }
	...
	{
		if(0 == is_full_mode)//zhaoqf_sh for manual test
		{
		do {
