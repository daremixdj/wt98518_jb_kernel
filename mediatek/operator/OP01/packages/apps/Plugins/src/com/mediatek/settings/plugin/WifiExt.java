...
public class WifiExt extends DefaultWifiExt implements AdapterView.OnItemSelectedListener {
    ...
    static final int WIFI_EAP_METHOD_SIM = 3;
    static final int WIFI_EAP_METHOD_AKA = 4;
...
//access point
    ...
    public void hideWifiConfigInfo(List<Object> lists) {
        if (lists != null) {
            int listSize =lists.size();
            if (listSize >= 4) {
                String ssid = (String)lists.get(0);
                int security = (Integer)lists.get(1);
                int networkId = (Integer)lists.get(2);
                boolean edit = (Boolean)lists.get(3);
                ...
                if ((networkId == WifiConfiguration.INVALID_NETWORK_ID) || (networkId != WifiConfiguration.INVALID_NETWORK_ID && edit)) {
                    if (FeatureOption.MTK_EAP_SIM_AKA) {
            	        for (int i = WIFI_EAP_LAYOUT_OFF; i <listSize-2; i++) {
	                        ((View)lists.get(i)).setVisibility(View.GONE);
	                    }
	                    LinearLayout advanced = ((LinearLayout)lists.get(listSize-1));
						int count = advanced.getChildCount();
	                    Xlog.d(TAG, "WifiExt eap count" + count);
	                    for (int j = WIFI_EAP_SKIP_NUM; j <count; j++) {
	                        ((View)advanced.getChildAt(j)).setVisibility(View.GONE);
	                    }
                    } else {
                        for (int i = WIFI_EAP_LAYOUT_OFF; i <listSize-1; i++) {
	                        ((View)lists.get(i)).setVisibility(View.GONE);
	                    }
                    }
                }
    ...
    public int getEapMethodbySpinnerPos(int spinnerPos, String ssid, int security) {
        Xlog.d(TAG, "WifiExt eapPos " + spinnerPos);
        if (FeatureOption.MTK_EAP_SIM_AKA) {
            if (ssid != null && (CMCC_AUTO_SSID.equals(ssid) && SECURITY_EAP == security)) {
                if (spinnerPos == 2) {
