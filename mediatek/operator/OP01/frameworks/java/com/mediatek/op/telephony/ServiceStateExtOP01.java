...
//changed by guqinghuan req:414,according MTK_FAQ_DOC
    public int mapGsmSignalLevel(boolean is3G,int asu,int dbm){
        int level;		
        ...
        if(FeatureOption.WT_HTC_REQ)
        {
	        if(is3G)
           { 
	        	if(dbm < -111) level = SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN; //0 signal
		        else if (dbm >= -90) level = SignalStrength.SIGNAL_STRENGTH_GREAT; //4 signal
		        else if (dbm >= -100)  level = SignalStrength.SIGNAL_STRENGTH_GOOD;   //3 signal
		        else if (dbm >= -108)  level = SignalStrength.SIGNAL_STRENGTH_MODERATE;  //2 signal
		        else level = SignalStrength.SIGNAL_STRENGTH_POOR; 		//1 signal
					}
					else
					{
	       	  if(dbm < -111) level = SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN; //0 signal
		        ...
		        else level = SignalStrength.SIGNAL_STRENGTH_POOR; 		//1 sign
					}        	
        	}
        else{	
        if(is3G)
        {
	        if (dbm < -111) level = SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN; //0 signal
	        else if (dbm >= -91) level = SignalStrength.SIGNAL_STRENGTH_GREAT; //4 signal
	        else if (dbm >= -98)  level = SignalStrength.SIGNAL_STRENGTH_GOOD;   //3 signal
	        else if (dbm >= -105)  level = SignalStrength.SIGNAL_STRENGTH_MODERATE;  //2 signal
	        else level = SignalStrength.SIGNAL_STRENGTH_POOR; 		//1 signal
				}
				else
				{
	        if (dbm < -109) level = SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN; //0 signal
	        else if (dbm >= -89) level = SignalStrength.SIGNAL_STRENGTH_GREAT; //4 signal
	        else if (dbm >= -97)  level = SignalStrength.SIGNAL_STRENGTH_GOOD;   //3 signal
	        else if (dbm >= -103)  level = SignalStrength.SIGNAL_STRENGTH_MODERATE;  //2 signal
        	else level = SignalStrength.SIGNAL_STRENGTH_POOR;
				}
			}

	Log.i(TAG,"mapGsmSignalLevel is3G=" + is3G + "asu" + asu + "dbm=" + dbm + "level" + level); 		
    ...
    public int mapGsmSignalDbm(int GsmRscpQdbm,int asu){
        int dBm;
//changed by guqinghuan req:414,according MTK_FAQ_DOC
        if(GsmRscpQdbm < 0 && GsmRscpQdbm != -1)
