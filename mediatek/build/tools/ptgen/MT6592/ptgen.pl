...
my $DebugPrint = "yes";  # yes for debug; no for non-debug
my $PRODUCT_OUT;
my $ptgen_location;
...
my $PARTITION_DEFINE_C    = "mediatek/kernel/drivers/dum-char/partition_define.c";
my $COMBO_NAND_TOOLH      = "mediatek/external/mtd-utils/ubi-utils/combo_nand.h";

my $PreloaderC         = "$custom_out_prefix/preloader/cust_part.c";
my $KernelH            = "$custom_out_prefix/kernel/core/src/partition.h";
...
sub InitGlobalValue
{
	$ArgList{ADD_BLINKFEED_PARTITION}  = $ENV{WINGTECH_ADD_BLINKFEED_PARTITION};
    ...
    if ($ArgList{ADD_BLINKFEED_PARTITION} eq "yes")
    {
    	$kernel_alias{"BLINKFEED"} = "blinkfeed";
    }
    ...
    $custom_out_prefix  = "mediatek/custom/$ArgList{PROJECT}";
    $configs_out_prefix = "mediatek/config/$ArgList{PROJECT}";

    #download files
   if ($ArgList{ADD_BLINKFEED_PARTITION1} eq "yes")
{
    %download_files = (
...
sub GenPartSizeFile_iniFile
{
    ...
    my $emmc_part_size = "mediatek/config/$ArgList{PROJECT}/configs/EMMC_partition_size.mk";
    if (-e $emmc_part_size)
    {
        if (!unlink $emmc_part_size)
        {
            &error_handler("remove $emmc_part_size fail ", __FILE__, __LINE__);
        }
    }
