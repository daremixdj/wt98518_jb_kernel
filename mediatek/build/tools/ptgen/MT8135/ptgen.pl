...
sub GenHeaderFile ()
{
    ...
    my $temp ;
	my $t;
	my $headfilepath="mediatek/custom/$PROJECT/common/";
	unless(-e $headfilepath){
		{`mkdir -p $headfilepath`;}	
	}
...
sub GenScatFile ()
{
	...
	if($EMMC_SUPPORT eq "yes" && $RESERVED_FIELD[$iter] == 1 && $PLATFORM eq "MT8135"){
			$temp .= "RSV_";
	}
