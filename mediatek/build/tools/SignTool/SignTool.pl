...
##########################################################
# Copy Scatter File and Preloader to Signed Folder
##########################################################
print "\n\n";
print "**************************************************\n";
print " Copy Scatter File and Preloader to Signed Folder \n";
print "**************************************************\n";
my $chip_id = substr $prj, 0, 6;
$chip_id = uc $chip_id;
my $src_file_name = $chip_id."_Android_scatter.txt";
my $src_file_path = "out/target/product/$prj/$src_file_name";
my $dest_file_signed_path = "out/target/product/$prj/signed_bin/$src_file_name";
my $scatter_file_signed_path = $dest_file_signed_path;
print "Copy SCATTER_FILE : $src_file_name to signed-bin folder\n";
# if the file does not exit, exit immediately. (for chip before mt6592)
system("cp -rf $src_file_path ./$dest_file_signed_path") == 0 or exit 0;
my $src_file_path = "out/target/product/$prj/preloader_$prj.bin";
my $dest_file_signed_path = "out/target/product/$prj/signed_bin/preloader_$prj.bin";
print "Copy $dest_file_signed_path to signed-bin folder\n";
system("cp -rf $src_file_path ./$dest_file_signed_path") == 0 or print "copy preloader file to signed folder fail\n";



##########################################################
# Replace file_name in Scatter file with signed file_name
##########################################################
print "\n\n";
print "*********************************************************\n";
print " Replace file_name in Scatter file with signed file_name \n";
print "*********************************************************\n";
my @imgs_need_sign = ("boot.img", "logo.bin", "recovery.img", "secro.img", "system.img", "userdata.img", "lk.bin", "cache.img");

foreach my $img (@imgs_need_sign) {
	my $signed_img = $img;
	$signed_img =~ s/\./-sign\./;
	
	system("sed -i 's/file_name:\ $img/file_name:\ $signed_img/g' $scatter_file_signed_path") == 0 or print "replace file_name:$img in scatter file fail";
}

my @imgs_need_sign = ("MBR", "EBR1", "EBR2");
foreach my $img (@imgs_need_sign) {
	my $signed_img = $img."-sign";
	
	system("sed -i 's/file_name:\ $img/file_name:\ $signed_img/g' $scatter_file_signed_path") == 0 or print "replace file_name:$img in scatter file fail";
}
print "Replace file_name Done!!\n";
