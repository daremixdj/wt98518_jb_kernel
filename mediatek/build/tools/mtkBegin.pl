...
#if (!-e $flag_subrel)
{
	...
	if (-d $srcDir)
	{
	  if ($ENV{"WT_CTA_SUPPORT"} eq "yes")
	  {
	    system("rsync -av --exclude=.svn --exclude=.git --exclude=.cvs --exclude=app --exclude=apptosign --exclude=appbackup $srcDir $dstDir > auto_sync_android.log 2>&1");
	  } 
	  else
	  {
	    system("rsync -av --exclude=.svn --exclude=.git --exclude=.cvs --exclude=apptosign $srcDir $dstDir > auto_sync_android.log 2>&1");
      }
	} 
}
