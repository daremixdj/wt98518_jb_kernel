...
ifeq ($(strip $(wildcard $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk)),)
MTK_CUSTGEN_TEMP := $(shell $(foreach a,$(CMD_ARGU),$(if $(filter 2,$(words $(subst =, ,$(a)))),$(a))) make MTK_CUSTGEN_ERROR=no -f mediatek/build/custgen.mk $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk)
endif
...
.PHONY: $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk
    endif
  endif
endif
...
#OTA_SCATTER_FILE := $(OUT_DIR)/target/product/$(PROJECT)/ota_scatter.txt
...
include mediatek/build/libs/pregen.mk
...
ifeq ($(filter generic banyan_addon banyan_addon_x86,$(PROJECT)),)
  MTK_DEPENDENCY_PREGEN_BEFORE_ANDROID   := codegen
else
...
pregen: $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk $(MTK_DEPENDENCY_PREGEN_LIST)
preclean: $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk $(MTK_DEPENDENCY_PRECLEAN_LIST)
codegen: $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk drvgen btcodegen cgen
...
check-dep: $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk
...
# We still need ProjectConfig.mk after cleanall in new
	$(hide) make MTK_CUSTGEN_ERROR=no -f mediatek/build/custgen.mk $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk $(DEAL_STDOUT_CUSTGEN)
endif
...
ifeq ($(strip $(WINGTECH_ADD_BLINKFEED_PARTITION1)), yes)
ANDROID_NATIVE_TARGETS := \
         ...
         factoryimage userdataimage userdataimage-nodeps customimage  blinkfeedimage
...
$(ANDROID_NATIVE_TARGETS): $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk custgen $(filter $(OUT_DIR)/target/product/$(PROJECT)/obj/include/dfo/%,$(MTK_DEPENDENCY_PREGEN_BEFORE_ANDROID)) $(ALLJAVAOPTFILES)
	$(hide) \
...
.PHONY: mm
mm: $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk $(MTK_ALL_CUSTGEN_FILES)
...
custgen: $(MTK_ROOT_CONFIG_OUT)/ProjectConfig.mk
	$(hide) echo $(SHOWTIME) $@ing...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)$@.log
	$(hide) make MTK_CUSTGEN_ERROR=no -f mediatek/build/custgen.mk $(DEAL_STDOUT_CUSTGEN) && \
...
emigen:
ifneq (,$(filter MT6592 MT8135 MT6575 MT6577 MT6589 MT8135 MT6582 MT6572,$(MTK_PLATFORM)))
ifneq ($(PROJECT), generic)
