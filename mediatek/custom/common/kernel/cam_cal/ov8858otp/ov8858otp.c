...
extern int read_otp_ov8858_VCM(u8 *p);
extern int OV8858_VCM_start,OV8858_VCM_end;
#define READ_AFINF 0X01
#define READ_AFMACRO 0X02

extern int g_ov8858_module_id;
extern int g_ov8858truly_module_id;
#define OV8858_OTP_ID	(0x11)
#define OV8858TRULY_OTP_ID	(0x2)
#define OV8858TRULY_START_OFFSET 60
#define OV8858TRULY_END_OFFSET 60
#define OV8858_START_OFFSET 60
#define OV8858_END_OFFSET 60
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
			...
			if(READ_AFINF ==  ptempbuf->u4Offset){
				if(g_ov8858_module_id == OV8858_OTP_ID)
				{
					value = OV8858_VCM_start < OV8858_START_OFFSET? 0 : (OV8858_VCM_start - OV8858_START_OFFSET);
				}
				else if(g_ov8858truly_module_id == OV8858TRULY_OTP_ID)
				{
					value = OV8858_VCM_start <OV8858TRULY_START_OFFSET? 0 : (OV8858_VCM_start - OV8858TRULY_START_OFFSET);
				}
				else
				{
					CAM_CALDB("[CAM_CAL] OTP ID not correct!\n");
				}
			}else{
				if(g_ov8858_module_id == OV8858_OTP_ID)
				{
					value = OV8858_VCM_end + OV8858_END_OFFSET;
				}
				else if(g_ov8858truly_module_id == OV8858TRULY_OTP_ID)
				{
					value = OV8858_VCM_end + OV8858TRULY_END_OFFSET;
				}
				else
				{
					CAM_CALDB("[CAM_CAL] OTP ID not correct!\n");
			...
			CAM_CALDB("[S24CAM_CAL] After read Working buffer data  %d \n", value);
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&value , 4))
        {
            kfree(pBuff);
            kfree(pWorkingBuff);
...
static int CAM_CAL_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
	...
	CAM_CALDB("[S24CAM_CAL] CAM_CAL_Release\n");
