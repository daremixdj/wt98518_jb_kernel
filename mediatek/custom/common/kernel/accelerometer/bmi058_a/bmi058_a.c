...
#define BMA255_ACC_X_LSB__POS           2
#define BMA255_ACC_X_LSB__LEN           6
#define BMA255_ACC_X_LSB__MSK           0xFC
...
#ifdef CONFIG_SENSORS_BMI058
#define BMA255_ACC_Y_LSB__POS           2
#define BMA255_ACC_Y_LSB__LEN           6
#define BMA255_ACC_Y_LSB__MSK           0xFC
...
#ifdef CONFIG_SENSORS_BMI058
#define BMA255_ACC_Z_LSB__POS           2
#define BMA255_ACC_Z_LSB__LEN           6
#define BMA255_ACC_Z_LSB__MSK           0xFC
...
static unsigned char *I2CDMABuf_va = NULL;           // added by yangbo 0114
static volatile unsigned int I2CDMABuf_pa = NULL;
/*----------------------------------------------------------------------------*/
static struct data_resolution bma255_data_resolution[1] = {
 /* combination by {FULL_RES,RANGE}*/
#ifdef CONFIG_SENSORS_BMI058
	{{ 0, 2}, 4096},   // dataformat +/-2g  in 14-bit resolution;  { 0, 2} = 0.2= (2*2*1000)/(2^14);  4096 = (2^14)/(2*2)
#else
...
/*----------------------------------------------------------------------------*/
#ifdef CONFIG_SENSORS_BMI058
static struct data_resolution bma255_offset_resolution = {{0, 2}, 4096};
#else
...
#else
static int
bma_i2c_send(struct i2c_client *client, const uint8_t *buf, int len)
{
	int i = 0;
	for(i = 0 ; i < len; i++)
	{
...
static int
bma_i2c_recv(struct i2c_client *client, uint8_t *buf, int len)
{
	int i = 0, err = 0;
...
int err;
		...
		mutex_lock(&i2c_lock);
		err = DMA_i2c_Read(client, &addr, 1, data, len);
		mutex_unlock(&i2c_lock);
...
/*
		...
		mutex_lock(&i2c_lock);
		err = bma_i2c_send(client, &addr, 1);
		if (err)
		{
				mutex_unlock(&i2c_lock);
				return err;
		...
		err = bma_i2c_recv(client, data, len);
		if (err)
		{
				mutex_unlock(&i2c_lock);
				return err;
	...
	struct i2c_msg msgs[2] = {
		...
		{
			.addr = client->addr,	.flags = I2C_M_RD, /*.ext_flag = I2C_WR_FLAG,*/
			.len = len,		.buf = data,
		}
	};
	int err;
			...
			u8 *data, u8 len)
{
		...
		mutex_lock(&i2c_lock);
		err = DMA_i2c_Read(client, buf, num, data, 0);
		mutex_unlock(&i2c_lock);
...
/*
		...
		mutex_lock(&i2c_lock);
		err = bma_i2c_send(client, buf, num);
		if (err)
		{
				mutex_unlock(&i2c_lock);
				return err;
...
static int BMA255_ReadData(struct i2c_client *client, s16 data[BMA255_AXES_NUM])
{
	...
	else
	{
...
#ifdef CONFIG_SENSORS_BMI058
		data[BMA255_AXIS_Y] = BMA255_GET_BITSLICE(buf[0], BMA255_ACC_Y_LSB)
		...
		data[BMA255_AXIS_Y] = data[BMA255_AXIS_Y] >> (sizeof(short)*8-(BMA255_ACC_Y_LSB__LEN
					+ BMA255_ACC_Y_MSB__LEN));
		data[BMA255_AXIS_X] = BMA255_GET_BITSLICE(buf[2], BMA255_ACC_X_LSB)
...
static int BMA255_ReadRawData_New(struct i2c_client *client, int buf[3])
{
	struct bma255_i2c_data *obj = (struct bma255_i2c_data*)i2c_get_clientdata(client);
	int res = 0;
	s16 databuf[BMA255_AXES_NUM];
...
static int BMA255_ReadSensorData_New(struct i2c_client *client, int buf[3], int bufsize)
{
	struct bma255_i2c_data *obj = (struct bma255_i2c_data*)i2c_get_clientdata(client);
	//u8 databuf[20];
	int acc[BMA255_AXES_NUM];
	...
	else
	{
		...
		buf[0]=acc[BMA255_AXIS_X];
		buf[1]=acc[BMA255_AXIS_Y];
		buf[2]=acc[BMA255_AXIS_Z];
...
static int bma255_get_fifo_framecount(struct i2c_client *client, unsigned char *framecount)
{
	...
	*framecount = BMA255_GET_BITSLICE(data, BMA255_FIFO_FRAME_COUNTER_S);
	//printk("a framecount=%d, data =%d\n", *framecount,data);//taodeliang
	return comres;
}
...
static ssize_t store_fifo_framecount_value(struct device_driver *ddri, char *buf, size_t count)
{
	...
	//printk("a fifo count=%d\n", obj->fifo_count);//taodeliang
	mutex_unlock(&obj->lock);
...
static ssize_t show_fifo_data_out_frame_value(struct device_driver *ddri, char *buf)
{
	...
	//printk("a fifo count is %d, max=%d\n", obj->fifo_count,MAX_FIFO_F_LEVEL * MAX_FIFO_F_BYTES);
	if (obj->fifo_count == 0) {
		return -EINVAL;
	}
	...
	if (f_len == 6) {
		...
		for (i = 0; i < obj->fifo_count; i++) {
			...
			databuf[BMA255_AXIS_Y] = ((unsigned char)fifo_data_out[i * f_len + 1] << 8 |
							(unsigned char)fifo_data_out[i * f_len + 0]) >> 2;
			databuf[BMA255_AXIS_X] = ((unsigned char)fifo_data_out[i * f_len + 3] << 8 |
							(unsigned char)fifo_data_out[i * f_len + 2]) >> 2;
			databuf[BMA255_AXIS_Z] = ((unsigned char)fifo_data_out[i * f_len + 5] << 8 |
							(unsigned char)fifo_data_out[i * f_len + 4]) >> 2;
#else
...
static int bma255_get_cali_offset(struct i2c_client *client,SENSOR_DATA *sensor_data)
{
	struct bma255_i2c_data *obj =
		(struct bma255_i2c_data *)i2c_get_clientdata(client);
	...
	for(i=0;i<20;i++)
	{
		...
		err= BMA255_ReadRawData_New(client,data);
		if (err) {
	...
	avg_z = sum_z/20;	
		...
		acc[obj->cvt.map[0]] =	obj->cvt.sign[0]*avg_x;
		acc[obj->cvt.map[1]] =	obj->cvt.sign[1]*avg_y;
		acc[obj->cvt.map[2]] =	obj->cvt.sign[2]*avg_z;
		acc[0] = acc[0] *
		...
		sensor_data->x = acc[0];
		sensor_data->y = acc[1];
		sensor_data->z = acc[2]-GRAVITY_EARTH_1000;
		return 0;
}
#if 0
static int bma255_get_cali_offset(struct i2c_client *client,SENSOR_DATA *sensor_data)
...
static long bma255_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_READ_SENSORDATA:
			...
			case GSENSOR_IOCTL_READ_SENSORDATA_NEW:
				data = (void __user *) arg;
				int buf[3]={0,0,0};
				if(data == NULL)
				{
				...
				BMA255_ReadSensorData_New(client, buf, BMA255_BUFSIZE);
				...
				break;
				case GSENSOR_SET_ACC_VALID://whf add start
				data = (void __user *) arg;
				
					if(copy_from_user(&obj->valid, data, sizeof(obj->valid)))
					{
						err = -EFAULT;
						break;
					}
				...
				data = (void __user *) arg;
					...
					if(enable == 1)
						err = BMA255_SetPowerMode(client,1);
					else
						err =BMA255_SetPowerMode(client,0);
					if (err)
						GSE_ERR("set power mode failed, err = %d\n", err);
					break;
			
				
				case GSENSOR_GET_ACC_RAW_DATA_FOR_CALI: 
					data = (void __user *) arg;
					if (data == NULL) {
						err = -EINVAL;
						break;
					}
					err = bma255_get_cali_offset(client,&sensor_data);
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			data = (void __user *) arg;
				int raw_data[3]={0,0,0};
			if(data == NULL)
			{
				err = -EINVAL;
				break;	  
			}
			err=BMA255_ReadRawData_New(client, raw_data);
			if (err) {
				err = -EFAULT;
				break;
			}
			...
			acc[obj->cvt.map[0]] =
				obj->cvt.sign[0]*raw_data[0];
			acc[obj->cvt.map[1]] =
				obj->cvt.sign[1]*raw_data[1];
			acc[obj->cvt.map[2]] =
				obj->cvt.sign[2]*raw_data[2];
			acc[0] = acc[0] *
				GRAVITY_EARTH_1000 / obj->reso->sensitivity;
			acc[1] = acc[1] *
				GRAVITY_EARTH_1000 / obj->reso->sensitivity;
...
//tad3sgh add --
	...
	// added by yangbo 0114
  I2CDMABuf_va = (u8 *)dma_alloc_coherent(NULL, 4096, &I2CDMABuf_pa, GFP_KERNEL);// 申请DMA空间  
	  if(!I2CDMABuf_va)
		{
...
static int bma255_i2c_remove(struct i2c_client *client)
{
	...
	if(err = hwmsen_detach(ID_ACCELEROMETER))
	{
		GSE_ERR("hwmsen_detach fail: %d\n", err);
	 				...
	 				client->ext_flag = client->ext_flag & (~I2C_DMA_FLAG); //去掉flag标记
