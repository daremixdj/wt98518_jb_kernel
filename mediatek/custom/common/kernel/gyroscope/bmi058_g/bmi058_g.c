...
#ifdef CONFIG_SENSORS_BMI058
		strcpy(obj->sensor_name, "bmi058 chip");
#else
		...
		struct device_attribute *attr, char *buf)
{
	...
	//printk("g fifo count is %d, max=%d\n", obj->fifo_count,MAX_FIFO_F_LEVEL * MAX_FIFO_F_BYTES);
	
  //printk("yangbo2==show_fifo_data_frame_value\n");
	if (obj->fifo_count == 0) {
		return -ENOENT;
	}
	...
	if (f_len == 6) {
		...
		for (i = 0; i < obj->fifo_count; i++) {
			...
			databuf[BMG_AXIS_Y] = ((unsigned char)g_fifo_data_out[i * f_len + 1] << 8 |
							(unsigned char)g_fifo_data_out[i * f_len + 0]);
			databuf[BMG_AXIS_X] = ((unsigned char)g_fifo_data_out[i * f_len + 3] << 8 |
