...
extern ALIVE_MAINSENSOR_STRUCT Alive_MainSensor;
...
#ifdef S5K3H2YXMIPI_USE_OTP

void S5K3H2YXMIPI_wordwrite_cmos_sensor(kal_uint16 addr, kal_uint32 para)
{
	char puSendCmd[4] = {(char)(addr >> 8) , (char)(addr & 0xFF) , (char)(para >> 8),	(char)(para & 0xFF) };
	iWriteRegI2C(puSendCmd , 4, S5K3H2YXMIPI_WRITE_ID);
}
...
//	BYTE val = 0;
//	int i;
	S5K3H2YXMIPI_wordwrite_cmos_sensor(0xFCFC, 0xD000);
	S5K3H2YXMIPI_write_cmos_sensor(0x3A1C, 0x00);   //disable test mode
	S5K3H2YXMIPI_write_cmos_sensor(0x0A00, 0x04);   //make initial state
	S5K3H2YXMIPI_write_cmos_sensor(0x0A02, zone);   //set the page 10 of OTP
 	S5K3H2YXMIPI_write_cmos_sensor(0x0A00, 0x01);   //set read mode
...
void stop_read_otp()
{
	S5K3H2YXMIPI_write_cmos_sensor(0x0A00, 0x00);   //Reset the NVM interface by writing 00h to 0xD0000A00
}
...
BYTE get_otp_flag(BYTE zone)
{
	...
	flag = S5K3H2YXMIPI_read_cmos_sensor(0x0A0B);
	stop_read_otp();
...
bool get_otp_date(BYTE zone) 
{
	...
	year  = S5K3H2YXMIPI_read_cmos_sensor(0x0A05);
	month = S5K3H2YXMIPI_read_cmos_sensor(0x0A06);
	day   = S5K3H2YXMIPI_read_cmos_sensor(0x0A07);
...
BYTE get_otp_module_id(BYTE zone)
{
	...
	module_id = S5K3H2YXMIPI_read_cmos_sensor(0x0A0A);

	stop_read_otp();

	//printk("Module ID: 0x%02x.",module_id);
...
BYTE get_otp_vcm_id(BYTE zone)
{
	...
	vcm_id = S5K3H2YXMIPI_read_cmos_sensor(0x0A0C);
 ...
 if(r_ratio >= 512 )
 ...
 else 			
  {
   if(b_ratio >= 512)
	...
	S5K3H2YXMIPI_wordwrite_cmos_sensor(GAIN_RED_ADDR, R_GAIN);
	S5K3H2YXMIPI_wordwrite_cmos_sensor(GAIN_BLUE_ADDR, B_GAIN);	
...
bool get_otp_wb(BYTE zone)
{
	...
	if(!start_read_otp(zone))
	{
  	...
  	golden_rg = 0x02EB;  //0x0303
  	golden_bg = 0x029E;  //0x028B
  	
	temph = S5K3H2YXMIPI_read_cmos_sensor(0x0A04);  
	templ = S5K3H2YXMIPI_read_cmos_sensor(0x0A05);   
	current_rg  = (USHORT)templ + ((USHORT)temph << 8);
	//printk("get_otp_wb  0x0A04= 0x%02x , 0x0A05 =0x%02x  \n ",temph,templ);
		
	temph = S5K3H2YXMIPI_read_cmos_sensor(0x0A06);  
	templ = S5K3H2YXMIPI_read_cmos_sensor(0x0A07);
	current_bg  = (USHORT)templ + ((USHORT)temph << 8);
	//printk("get_otp_wb  0x0A06= 0x%02x , 0x0A07 =0x%02x  \n",temph,templ);
...
bool otp_update()
{
	...
	MID = get_otp_module_id(zone);
	if((MID != Oflim_ID)&&(MID != Sunny_ID))
	{
		printk("No Oflim or Sunny Module !");
		return 0;
	}
	//get_otp_date(zone);
...
void S5K3H2YXMIPI_Set_2M_PVsize(void)
{
...
#ifdef S5K3H2YXMIPI_USE_OTP	
		//sandy add for OTP shading start	
		S5K3H2YXMIPI_write_cmos_sensor(0x3210, 0x81);		 // shade clock enable
		S5K3H2YXMIPI_write_cmos_sensor(0x3200, 0x08);			// shade decompression disable
		//sandy add for OTP shading end 	
		mdelay(1);
#endif
...
#ifdef S5K3H2YXMIPI_USE_OTP	
		//S5K3H2YXMIPI_write_cmos_sensor(0x3210, 0x01);		//Pedestal  sandy modify
#else
		...
		//S5K3H2YXMIPI_write_cmos_sensor(0x0105, 0x01);
			...
			//PLL	EXCLK 24MHz, vt_pix_clk_freq_mhz=129.6,op_sys_clk_freq_mhz=648			   
			//Address	Data	Comment 																   
		...
		S5K3H2YXMIPI_write_cmos_sensor(0x30CC, 0xB0);	//DPHY_band_ctrl 640¡ã690MHz											   
		...
		S5K3H2YXMIPI_write_cmos_sensor(0x0341, 0xEC);																		   
...
#ifdef S5K3H2YXMIPI_USE_OTP			
		//SANDY MODify		
		S5K3H2YXMIPI_write_cmos_sensor(0x3202, 0x34);		//sh4ch_blk_width																   
		S5K3H2YXMIPI_write_cmos_sensor(0x3203, 0x4D);		//sh4ch_blk_height															   
		S5K3H2YXMIPI_write_cmos_sensor(0x3204, 0x04);
		S5K3H2YXMIPI_write_cmos_sensor(0x3205, 0xEC);																		   
		S5K3H2YXMIPI_write_cmos_sensor(0x3206, 0x03);																		   
		S5K3H2YXMIPI_write_cmos_sensor(0x3207, 0x54);
		
		S5K3H2YXMIPI_write_cmos_sensor(0x3201, 0x00);		//Shading On	
		mDELAY(20);															   
		S5K3H2YXMIPI_write_cmos_sensor(0x3200, 0x28);			// shade decompression auto start															   
		S5K3H2YXMIPI_write_cmos_sensor(0x3A2D, 0x00);  	// Auto Read Off
		S5K3H2YXMIPI_write_cmos_sensor(0x3A2D, 0x01);  	// Auto Read ON
		mDELAY(50);	
#endif
...
{	
	   ...
	   //PLL³]©w	   EXCLK 24MHz, vt_pix_clk_freq_mhz=129.6,op_sys_clk_freq_mhz=648			   
	   //Address   Data    Comment			   
	   S5K3H2YXMIPI_write_cmos_sensor(0x0305, 0x04);   //pre_pll_clk_div = 4			   
	   S5K3H2YXMIPI_write_cmos_sensor(0x0306, 0x00);   //pll_multiplier 			   
	   S5K3H2YXMIPI_write_cmos_sensor(0x0307, 0x6C);   //pll_multiplier  = 108			   
...
#ifdef S5K3H2YXMIPI_USE_OTP			
		//SANDY MODify
		S5K3H2YXMIPI_write_cmos_sensor(0x3202, 0x67);		 //sh4ch_blk_width																   
		S5K3H2YXMIPI_write_cmos_sensor(0x3203, 0x9A);			//sh4ch_blk_height															   
		S5K3H2YXMIPI_write_cmos_sensor(0x3204, 0x02);
		S5K3H2YXMIPI_write_cmos_sensor(0x3205, 0x7C);																		   
		S5K3H2YXMIPI_write_cmos_sensor(0x3206, 0x01);																		   
		S5K3H2YXMIPI_write_cmos_sensor(0x3207, 0xAA);
...
static kal_uint16 S5K3H2YXMIPI_power_on(void)
{
	...
	for(i=0;i<3;i++)
	{
		...
		if(S5K3H2YXMIPI_sensor_id == S5K3H2YXMIPI_SENSOR_ID)
			break;
	}
...
UINT32 S5K3H2YXMIPIGetSensorID(UINT32 *sensorID) 
{
	...
	*sensorID = S5K3H2YXMIPI_power_on();  
	...
	Alive_MainSensor.ID = S5K3H2YXMIPI_SENSOR_ID;
#ifdef S5K3H2YXMIPI_USE_OTP	
	otp_3h2_read_MID_VCMID();
#endif
...
UINT32 S5K3H2YXMIPIGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId,
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{ 
	SENSORDB("[S5K3H2MIPI]enter S5K3H2MIPIGetInfo function\n");
	switch(ScenarioId){
			case MSDK_SCENARIO_ID_CAMERA_ZSD:
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG://hhl 2-28
				pSensorInfo->SensorFullResolutionX=S5K3H2YXMIPI_FULL_ACTIVE_PIXEL_NUMS - 32;
				pSensorInfo->SensorFullResolutionY=S5K3H2YXMIPI_FULL_ACTIVE_LINE_NUMS - 6-12-38;
				pSensorInfo->SensorStillCaptureFrameRate=25;

			break;//hhl 2-28
			case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
				pSensorInfo->SensorPreviewResolutionX=S5K3H2YXMIPI_PV_ACTIVE_PIXEL_NUMS - 12;
				pSensorInfo->SensorPreviewResolutionY=S5K3H2YXMIPI_PV_ACTIVE_LINE_NUMS - 20;
				pSensorInfo->SensorCameraPreviewFrameRate=30;
			break;
		default:
        pSensorInfo->SensorPreviewResolutionX=S5K3H2YXMIPI_PV_ACTIVE_PIXEL_NUMS - 12;
    ...
    switch (ScenarioId)
    {
		...
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
            ...
            pSensorInfo->SensorPixelClockCount= 3;
            pSensorInfo->SensorDataLatchCount= 2;
            pSensorInfo->SensorGrabStartX = S5K3H2YXMIPI_FULL_GRAB_X;	//2*IMX134MIPI_IMAGE_SENSOR_PV_STARTX; 
            pSensorInfo->SensorGrabStartY = S5K3H2YXMIPI_FULL_GRAB_Y;	//2*IMX134MIPI_IMAGE_SENSOR_PV_STARTY;          			
            pSensorInfo->SensorMIPILaneNumber = SENSOR_MIPI_2_LANE;			
            pSensorInfo->MIPIDataLowPwr2HighSpeedTermDelayCount = 0; 
            pSensorInfo->MIPIDataLowPwr2HighSpeedSettleDelayCount = 14; 
            pSensorInfo->MIPICLKLowPwr2HighSpeedTermDelayCount = 0; 
	...
	memcpy(pSensorConfigData, &S5K3H2YXMIPISensorConfigData, sizeof(MSDK_SENSOR_CONFIG_STRUCT));
	spin_unlock(&s5k3h2yxmipiraw_drv_lock);
    SENSORDB("[S5K3H2YXMIPI]exit S5K3H2YXMIPIGetInfo function\n");
    return ERROR_NONE;
} 
