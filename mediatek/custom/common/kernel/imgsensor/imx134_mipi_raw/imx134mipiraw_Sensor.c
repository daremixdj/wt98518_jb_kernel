...
#define SUPPORT_IMX134_OTP
#define IMX134FOXCONN_MODULE_ID			0x01
...
#ifdef SUPPORT_IMX134_OTP
extern int iReadLSCData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata);
extern int iReadModuleID(unsigned char * pinputdata);
#endif
...
#ifdef SUPPORT_IMX134_OTP
iReadLSCData(0,280,&lsc_data);
for(i=0;i<280;i++)
{
   IMX134MIPI_write_cmos_sensor(0x4800+i,lsc_data[i]);
   //SENSORDB("IMX134MIPI_globle_setting:%d,0x%x \n",i,lsc_data[i]);
...
//shading_control_value=IMX134MIPI_read_cmos_sensor(0x4500);
 //  SENSORDB("shading_control_value:0x%x,0x%x \n",shading_control_value,shading_control_ori_value);

  // if(shading_control_value==0x1F)
   	// IMX134MIPI_write_cmos_sensor(0x4500,shading_control_ori_value);
...
UINT32 IMX134MIPIGetSensorID(UINT32 *sensorID) 
{
	...
	SENSORDB("[IMX134MIPI FOXCONN]enter IMX134MIPIGetSensorID function\n");
    // check if sensor ID correct
    do {		
	   *sensorID =(kal_uint16)(((IMX134MIPI_read_cmos_sensor(0x0016)&&0x0f)<<8) | IMX134MIPI_read_cmos_sensor(0x0017)); 
	...
	SENSORDB("[IMX134MIPI_FOXCONN]*sensorID =%x\n",*sensorID);
...
#ifdef SUPPORT_IMX134_OTP
	if(iReadModuleID(&ModuleID) || IMX134FOXCONN_MODULE_ID != ModuleID){
		*sensorID = 0xFFFFFFFF; 
		SENSORDB("[IMX134MIPI FOXCONN] varify foxconn module id fail\n");
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	SENSORDB("[IMX134MIPI_FOXCONN] ModuleID =%x\n",ModuleID);
#endif
	SENSORDB("[IMX134MIPI FOXCONN]exit IMX134MIPIGetSensorID function\n");
    return ERROR_NONE;
}
