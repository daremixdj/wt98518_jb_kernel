...
/*****************************  OTP Feature  **********************************/
#define SUPPORT_IMX111_OTP

#ifdef SUPPORT_IMX111_OTP

// R/G and B/G of typical camera module is defined here
static int RG_Ratio_Typical = 0x282;
static int BG_Ratio_Typical = 0x262;
...
struct otp_struct {
	...
	int golden_rg_ratio;
	int golden_bg_ratio;
	int golden_GbGr_ratio;
	int ROI_R[9];
	int ROI_B[9];
};

static int update_wb(struct otp_struct *otp_info)
{
int RoverG_dec_base=RG_Ratio_Typical;
int BoverG_dec_base=BG_Ratio_Typical;
int rg_ratio = (*otp_info).rg_ratio;
int bg_ratio = (*otp_info).bg_ratio;
int gr_gain, gb_gain, r_gain, b_gain, g_gain;
int R_test_H3,R_test_L8,B_test_H3,B_test_L8,G_test_H3,G_test_L8;
...
if(bg_ratio < BG_Ratio_Typical)
{
	...
	{
		g_gain = 0x100;
		b_gain = 0x100 * BG_Ratio_Typical / bg_ratio;
		r_gain = 0x100 * RG_Ratio_Typical / rg_ratio;
	}
	else
	{
		r_gain = 0x100;
		g_gain = 0x100 * rg_ratio / RG_Ratio_Typical;
...
else
{
	...
	{
		b_gain = 0x100;
		g_gain = 0x100 * bg_ratio / BG_Ratio_Typical;
		r_gain = g_gain * RG_Ratio_Typical / rg_ratio;
	}
	else
	{
		gb_gain = 0x100 * bg_ratio / BG_Ratio_Typical;
		gr_gain = 0x100 * rg_ratio / RG_Ratio_Typical;
		if(gb_gain > gr_gain )
		{
			b_gain = 0x100;
			g_gain = gb_gain;
	...
	R_test_H3 = (r_gain>>8)&0x0F;
	R_test_L8 = r_gain &0xFF;
	B_test_H3 = (b_gain>>8)&0x0F;
	B_test_L8 = b_gain &0xFF;
	G_test_H3 = (g_gain>>8)&0x0F;
	G_test_L8 = g_gain &0xFF;
	
	temp1=IMX111MIPI_read_cmos_sensor(0x020F);
	temp2=IMX111MIPI_read_cmos_sensor(0x0211);
	temp3=IMX111MIPI_read_cmos_sensor(0x0213);
	temp4=IMX111MIPI_read_cmos_sensor(0x0215);
	SENSORDB("IMX111_update_wb, 0x020F = 0x%x, 0x0211 = 0x%x, 0x0213 = 0x%x, 0x0215 = 0x%x\n", temp1,temp2,temp3,temp4);
	temp1=IMX111MIPI_read_cmos_sensor(0x020E);
	temp2=IMX111MIPI_read_cmos_sensor(0x0210);
	temp3=IMX111MIPI_read_cmos_sensor(0x0212);
	temp4=IMX111MIPI_read_cmos_sensor(0x0214);
	SENSORDB("IMX111_update_wb, 0x020E = 0x%x, 0x0210 = 0x%x, 0x0212 = 0x%x, 0x0214 = 0x%x\n", temp1,temp2,temp3,temp4);
	IMX111MIPI_write_cmos_sensor(0x020E,G_test_H3);
	IMX111MIPI_write_cmos_sensor(0x020F,G_test_L8);
	IMX111MIPI_write_cmos_sensor(0x0210,R_test_H3);
	IMX111MIPI_write_cmos_sensor(0x0211,R_test_L8);
	IMX111MIPI_write_cmos_sensor(0x0212,R_test_H3);
	IMX111MIPI_write_cmos_sensor(0x0213,B_test_L8);
	IMX111MIPI_write_cmos_sensor(0x0214,G_test_H3);
	IMX111MIPI_write_cmos_sensor(0x0215,G_test_L8);
	
	SENSORDB("IMX111_update_wb, 1 = 0x%x, 2 = 0x%x, 3 = 0x%x\n", R_test_H3,B_test_H3,G_test_H3);
	SENSORDB("IMX111_update_wb, 0x020F = 0x%x, 0x0211 = 0x%x, 0x0213 = 0x%x, 0x0215 = 0x%x\n", G_test_L8,R_test_L8,B_test_L8,G_test_L8);
	return 1;
}

static int check_otp_mid(struct otp_struct *otp_info)
{
	int midAddr[4] = {0x3500,0x3508,0x3510,0x3518};
	int bank,i;
	for(i=0;i<4;i++)
	{	
		bank = (midAddr[i]&0xff)/8;
		IMX111MIPI_write_cmos_sensor(0x34c9, bank);
		(*otp_info).module_integrator_id = IMX111MIPI_read_cmos_sensor(midAddr[i]);
		(*otp_info).product_year = IMX111MIPI_read_cmos_sensor((midAddr[i])+2);
		(*otp_info).product_month = IMX111MIPI_read_cmos_sensor((midAddr[i])+3);
		(*otp_info).product_day = IMX111MIPI_read_cmos_sensor((midAddr[i])+4);
		SENSORDB("IMX111_check_otp_mid, bank = %d, mid = 0x%x\n", bank, (*otp_info).module_integrator_id);
		if (((*otp_info).module_integrator_id) == 0)
			continue;
		else 
			break;
	}
	if ((i == 4)&&(((*otp_info).module_integrator_id) == 0))
		return 0;
	SENSORDB("IMX111_check_otp_mid success!\n");
	return 1;
}


static int check_otp_wb(struct otp_struct *otp_info)
{
	//¿ªÊ¼¼ì²â AWB 
	int wbAddr[3] = {0x3538,0x3550,0x3568};

	int bank,i,j;
	for(i=0;i<3;i++)
	{
		//WB
		bank = (wbAddr[i]&0xff)/8;
		IMX111MIPI_write_cmos_sensor(0x34c9, bank);
		(*otp_info).rg_ratio = IMX111MIPI_read_cmos_sensor(wbAddr[i]);
		(*otp_info).rg_ratio = (((*otp_info).rg_ratio)<<8)+IMX111MIPI_read_cmos_sensor((wbAddr[i])+1);
		(*otp_info).bg_ratio = IMX111MIPI_read_cmos_sensor((wbAddr[i])+2);
		(*otp_info).bg_ratio = (((*otp_info).bg_ratio)<<8)+IMX111MIPI_read_cmos_sensor((wbAddr[i])+3);
		(*otp_info).GbGr_ratio = IMX111MIPI_read_cmos_sensor((wbAddr[i])+4);
		(*otp_info).GbGr_ratio = (((*otp_info).GbGr_ratio)<<8)+IMX111MIPI_read_cmos_sensor((wbAddr[i])+5);
		SENSORDB("IMX111_check_otp_wb, bank:%d, r/g:0x%x, b/g:0x%x, Gb/Gr:0x%x\n", bank, (*otp_info).rg_ratio, (*otp_info).bg_ratio, (*otp_info).GbGr_ratio);

		if (((*otp_info).rg_ratio) == 0)
			continue;
		else 
			break;
	}
	if ((i == 3)&&(((*otp_info).rg_ratio) == 0))
		return 0;

	if(!(update_wb(otp_info)))
		return 0;
	//AWB¼ì²â³É¹¦
	
	SENSORDB("check_otp_wb success!\n");
	return 1;
}
...
static int update_otp_info()
{
	...
	if(!(check_otp_mid(&current_otp)))
	{
		SENSORDB("IMX111_check_otp_mid fail!\n");
		//¼ì²âMIDÊ§°Ü
		goto OTPERR;
	}

	if (!(check_otp_wb(&current_otp)))
	{
		SENSORDB("check_otp_wb fail!\n");
		//¼ì²âAWB & ColorShadingÊ§°Ü
		goto OTPERR;
	}
	flag = 1;
	OTPERR:
