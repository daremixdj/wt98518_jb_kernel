		...
		unsigned int address)
{
	...
	return (retval == PAGE_SELECT_LEN) ? retval : -EIO;
	 ...
	 //printk("s3203 tpd_i2c_write_byte_data ret=%d\n",ret);
		...
		const struct i2c_device_id *dev_id)
{
	...
	#include <linux/hardware_info.h>
		hardwareinfo_set_prop(HARDWARE_TP, "S3402");
	}
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	printk("synaptics_rmi4_early_suspend staying_awake=%d",rmi4_data->staying_awake);
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
			container_of(h, struct synaptics_rmi4_data,
			early_suspend);
	printk("synaptics_rmi4_late_resume staying_awake=%d",rmi4_data->staying_awake);
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	printk("synaptics_rmi4_suspend staying_awake=%d",rmi4_data->staying_awake);
	if (rmi4_data->staying_awake)
		return 0;
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	printk("synaptics_rmi4_resume staying_awake=%d",rmi4_data->staying_awake);
	if (rmi4_data->staying_awake)
		return 0;
