...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
	{//CALIBRATION_LAYOUT_SLIM_LSC1 without Defect //data sheet of excel : "Slim"
		0xFFFFFFFF, 0x010200FF, CAM_CAL_SINGLE_EEPROM_DATA,
		{
	...
	{//CALIBRATION_LAYOUT_N3D //data sheet of excel : "3D_EEPROM 8M+2M _0A_2"
		0xFFFFFFFF, 0x020A00FF,CAM_CAL_N3D_DATA,
		{
	...
	{//CALIBRATION_LAYOUT_SUNY
		0xFFFFFFFF, 0x796e7573, CAM_CAL_SINGLE_EEPROM_DATA,
		{		
			{0x00000001, 0x00000000, 0x00000000, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
			{0x00000001, 0x00000000, 0x00000000, DoCamCalPartNumber}, //CAMERA_CAM_CAL_DATA_PART_NUMBER
			{0x00000001, 0x0000000C, 0x00000284, DoCamCalSingleLsc}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE
	...
	{//CALIBRATION_LAYOUT_EEPROM_IMX134_Wingtech
		0x00000001, 0x0000ffff, CAM_CAL_SINGLE_EEPROM_DATA,
		{		
			...
			{0x00000000, 0x00000000, 0x00000000, DoCamCalSingleLsc}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE
			{0x00000001, 0x00000004, 0x00000006, DoCamCalAWBGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN
			{0x00000000, 0x00000000, 0x00000000, DoCamCal3DGeo}  //CAMERA_CAM_CAL_DATA_3D_GEO
		}
	}	
...
static UINT32 DoCamCalAWBGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    stCAM_CAL_INFO_STRUCT  cam_calCfg;
    PCAM_CAL_DATA_STRUCT pCamCalData = (PCAM_CAL_DATA_STRUCT)pGetSensorCalData;
    u16 Current_Ratio_RG,Current_Ratio_BG,Current_Ratio_GrGb;
    double Current_Ratio_GR,Current_Ratio_GbGr,Current_Ratio_GB;
    u8 Current_Ratio[6];	
    UINT32 ioctlerr;
    UINT32 err =  CamCalReturnErr[pCamCalData->Command];     
                ...
                ////AWB////
                    ...
                    CAM_CAL_LOG_IF(dumpEnable,"Read AWB OTP value\n");
                               ...
                               start_addr=0x00000004;
                               
                               cam_calCfg.u4Offset = 4;
                               cam_calCfg.u4Length = 6;
                               cam_calCfg.pu1Params = (u8 *)&Current_Ratio;            
                               ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
                              // CAM_CAL_LOG_IF(dumpEnable,"[Current_R/Gr_Address] = 0x%x\n", start_addr+0);
                               CAM_CAL_LOG_IF(dumpEnable,"Current_Ratio_RG_H = 0x%x\n", Current_Ratio[0]);
                              CAM_CAL_LOG_IF(dumpEnable,"Current_Ratio_RG_L = 0x%x\n", Current_Ratio[1]);
							  							CAM_CAL_LOG_IF(dumpEnable,"Current_Ratio_BG_H = 0x%x\n", Current_Ratio[2]);
							  							CAM_CAL_LOG_IF(dumpEnable,"Current_Ratio_BG_L = 0x%x\n", Current_Ratio[3]);
							  							CAM_CAL_LOG_IF(dumpEnable,"Current_Ratio_GrGb_H = 0x%x\n", Current_Ratio[4]);
							  							CAM_CAL_LOG_IF(dumpEnable,"Current_Ratio_GrGb_L = 0x%x\n", Current_Ratio[5]);
 															...
 															Current_Ratio_RG = Current_Ratio[0] << 8 | Current_Ratio[1];
 															Current_Ratio_BG = Current_Ratio[2] << 8 | Current_Ratio[3];
 															Current_Ratio_GrGb = Current_Ratio[4] << 8 | Current_Ratio[5];
                               ...
                               if(Current_Ratio_RG==0 || Current_Ratio_BG==0)
                               {
                               ...
                               Current_Ratio_GR  =(double) 1024.0/ Current_Ratio_RG;
                               Current_Ratio_GbGr  =(double)  1024.0/ Current_Ratio_GrGb;
                               Current_Ratio_GB  =(double) 1024.0/ Current_Ratio_BG;
                ...
                pCamCalData->Single2A.S2aAwb.rGoldGainu4R = (u32)(Current_Ratio_GR*512);//(Gold_R_G_Value*512);
                pCamCalData->Single2A.S2aAwb.rGoldGainu4G = (u32)(Current_Ratio_GbGr* 512);
                pCamCalData->Single2A.S2aAwb.rGoldGainu4B = (u32)(Current_Ratio_GB*512);//(Gold_B_G_Value*512);                                                                                       
...
UINT32 DoCamCalLayoutCheck(void)
{
    ...
    if ((gIsInitedCamCal==CAM_CAL_LAYOUT_RTN_QUEUE) && (CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0]) == 0))
    {
        ...
        for (i = 0; i< MAX_CALIBRATION_LAYOUT_NUM; i++)
        {
            ...
            {
                CheckID = 0x00000000;
                cam_calCfg.u4Offset = CalLayoutTbl[i].CalItemTbl.StartAddr;
                cam_calCfg.u4Length = CalLayoutTbl[i].CalItemTbl.BlockSize;
