...
*
*****************************************************************************/
#define LOG_TAG "CamCalCamCalOV8858"
...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
	...
	{//CALIBRATION_LAYOUT_SENSOR_OTP, OV8858 AF
...
static UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    { 
        ...
        {
			pCamCalData->Single2A.S2aVer = 0x01;
            pCamCalData->Single2A.S2aBitEn =0x01; //(0x03 & AWBAFConfig);
			CAM_CAL_LOG_IF(dumpEnable,"S2aBitEn=0x%x", pCamCalData->Single2A.S2aBitEn);
            pCamCalData->Single2A.S2aAfBitflagEn =0x03; //(0x0C & AWBAFConfig);// //Bit: step 0(inf.), 1(marco), 2, 3, 4,5,6,7

            ////AF////
            cam_calCfg.u4Offset = READ_AFINF;
            ...
            cam_calCfg.u4Offset = READ_AFMACRO;
            cam_calCfg.u4Length = 2;
...
*
*******************************************************************************/
UINT32 OV8858MIPIRAWTRULY_CAM_CALGetCalData(UINT32* pGetSensorCalData)
{	
    UCHAR cBuf[128] = "/dev/";	
    UCHAR HeadID[5] = "NONE";	
    UINT32 result = CAM_CAL_ERR_NO_DEVICE;
