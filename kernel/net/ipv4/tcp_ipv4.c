...
void tcp_v4_handle_retrans_time_by_uid(struct uid_err uid_e)
{
    ...
    //printk("[mmspb] call tcp_v4_handle_retrans_time_by_uid£¬ adftp_uid=%d\n", adftp_uid);
#if 0    
    if (adftp_uid_l != 0)
        skuid = (uid_t)adftp_uid_l;
    else
        return;
#endif
    for (bucket = 0; bucket < tcp_hashinfo.ehash_mask; bucket++) {
	    ...
	    icsk = inet_csk(sk);
            ...
            //printk("[mmspb] tcp_v4_handle_retrans_time_by_uid sk=0x%x, icsk=0x%x\n", sk, icsk);
            //if (retrans_time_by_uid_count > 0)
            //{
                  //struct tcp_sock *tp = tcp_sk(sk);
...
#if 0					
            {
                ...
                tp = tcp_sk(sk);
#if 0						
                printk("[mmspb] tcp_v4_handle_retrans_time_by_uid  snd_una=%u, snd_nxt=%u, high_seq=%u, packets_out=%u\n", 
                   tp->snd_una, tp->snd_nxt, tp->high_seq, tp->packets_out);	
#endif
            }
#endif
            //}
            //else
            //{
            //    sk_reset_timer(sk, &icsk->icsk_retransmit_timer, jiffies + 100*60);
            //    icsk->icsk_rto = TCP_RTO_MAX;					    
            //}
            //retrans_time_by_uid_count++;
    ...
    //printk("[mmspb] tcp_v4_handle_retrans_time_by_uid retrans_time_by_uid_count=%u\n", retrans_time_by_uid_count);
	//if (retrans_time_by_uid_count > 0)
	//{
	//	adftp_uid = adftp_uid_l;
	//	printk("[mmspb] tcp_v4_handle_retrans_time_by_uid adftp_uid=%u\n", adftp_uid);
	//}
}
