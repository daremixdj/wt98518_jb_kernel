		...
		struct pt_regs *regs)
{
	struct siginfo si;
        ...
        printk(KERN_ALERT "[TRAP] Current,pc:0x%lx,tid:0x%p,CP10:0x%x,CP11:0x%x,vfpstate:%p\n",regs->ARM_pc,thread,thread->used_cp[10],thread->used_cp[11],&(thread->vfpstate));
                
        printk(KERN_ALERT "[TRAP] r0:0x%lx  r1:0x%lx  r2:0x%lx  r3:0x%lx\n",regs->ARM_r0,regs->ARM_r1,regs->ARM_r2,regs->ARM_r3);
        printk(KERN_ALERT "[TRAP] r4:0x%lx  r5:0x%lx  r6:0x%lx  r7:0x%lx\n",regs->ARM_r4,regs->ARM_r5,regs->ARM_r6,regs->ARM_r7);
        printk(KERN_ALERT "[TRAP] r8:0x%lx  r9:0x%lx r10:0x%lx r11:0x%lx\n",regs->ARM_r8,regs->ARM_r9,regs->ARM_r10,regs->ARM_fp);
        printk(KERN_ALERT "[TRAP]r12:0x%lx r13:0x%lx r14:0x%lx r15:0x%lx\n",regs->ARM_ip,regs->ARM_sp,regs->ARM_lr,regs->ARM_pc);
#endif
#if 0
//VFP OPT
        if (thumb_mode(regs)){
            ...
            if (is_wide_instruction(curr_instr_fixup)) {
                ...
                if (count <= 3)
                {
 ...
 
                    {
                        regs->ARM_pc -= 2; 
                        count ++;
                        printk("[TRAP] fixup new PC:0x%lx, count:%d\n",regs->ARM_pc,count);
                        return;
                    }
                }
            }
