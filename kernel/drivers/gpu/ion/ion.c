...
static void _ion_buffer_destroy(struct kref *kref)
{
        ...
        if (t > 5000)
        	pr_err("_ion_buffer_destroy: %luus\n", t);
}
...
static void ion_buffer_remove_from_handle(struct ion_buffer *buffer)
{
        ...
        t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
        if (t > 5000)
        	pr_err("ion_buffer_remove_from_handle: %luus\n", t);
}
...
static void ion_handle_destroy(struct kref *kref)
{
        ...
        t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
        if (t > 5000)
        	pr_err("ion_handle_destroy: %luus\n", t);
}
                             ...
                             unsigned int flags)
{
		...
		//add by mtk begin for ALPS01474922
...
static void *ion_buffer_kmap_get(struct ion_buffer *buffer)
{
    ...
    MMProfileLogEx(ION_MMP_Events[PROFILE_MAP_KERNEL], MMProfileFlagStart, buffer->size, 0);
        vaddr = buffer->heap->ops->map_kernel(buffer->heap, buffer);
    MMProfileLogEx(ION_MMP_Events[PROFILE_MAP_KERNEL], MMProfileFlagEnd, buffer->size,(unsigned int)vaddr);
				     ...
				     const char *name)
{
    ...
    unsigned long t;
	    ...
	    t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
	    if (t > 5000)
	        pr_err("ion_client_create -- task_lock: %luus\n", t);
	    ...
	    t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
	    if (t > 5000)
	        pr_err("ion_client_create -- dev->lock: %luus\n", t);
...
/* Create checking_leakage entry */
	    ...
	    t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
	    if (t > 5000)
	        pr_err("ion_client_create -- ion_debugger: %luus\n", t);
	}
	#endif
...
void ion_client_destroy(struct ion_client *client)
{
    ...
    do_gettimeofday(&tv_e);
    t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
    pr_err("ion_client_destroy: %luus\n", t);
...
ion_debug_db_destroy_clentry(client->pid);
        ...
        do_gettimeofday(&tv_e);
	    t = (tv_e.tv_sec - tv_s.tv_sec)*1000000 + (tv_e.tv_usec - tv_s.tv_usec);
	    pr_err("ion_client_destroy -- ion_debugger on: %luus\n", t);
#endif
...
int ion_share_dma_buf_fd(struct ion_client *client, struct ion_handle *handle)
{
	...
	if(ion_debugger && (fd >= 0))
		...
		record_param.fd     = fd;
                ...
                ION_DEBUG_LOG(ION_DEBUG_TRACE,"[KERNEL ION_SHARE]\n");
                record_param.backtrace_num = get_kernel_backtrace((unsigned long *)record_param.backtrace);
		get_kernel_symbol((unsigned long *)record_param.backtrace,record_param.backtrace_num,&(record_param.kernel_symbol[0]));
                record_ion_info((int)1,&record_param);
		ION_DEBUG_LOG(ION_DEBUG_TRACE,"[KERNEL ION_SHARE] DONE\n");
...
struct ion_handle *ion_import_dma_buf(struct ion_client *client, int fd)
{
    ...
    MMProfileLogEx(ION_MMP_Events[PROFILE_IMPORT], MMProfileFlagStart, 1, 1);
        ...
        {
            if(dmabuf == ERR_PTR(-EBADF))
            {
                IONMSG("%s failed: bad fd! fd=%d\n", __FUNCTION__, fd);
            }
            else 
            {
                IONMSG("%s failed: fd is not ion_share_fd! fd=%d\n", __FUNCTION__, fd);
            }
