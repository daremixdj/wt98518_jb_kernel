...
void * allocate_record(unsigned int type,ion_sys_record_t *record_param)
{
	...
	switch(type)
	{
		case LIST_BUFFER:
                {
                        ...
                        {
                                ion_list_buffer_cachep = kmem_cache_create("buffer_record",sizeof(struct ion_buffer_record),0,0,NULL);
                ...
                case LIST_PROCESS:
                {
                        ...
                        {
                                ion_list_process_cachep = kmem_cache_create("process_record",sizeof(struct ion_process_record),0,0,NULL);
	        ...
	        case NODE_BUFFER:
		{
			...
			{
				ion_buffer_usage_cachep = kmem_cache_create("buffer_usage_record",sizeof(ion_buffer_usage_record_t),0,0,NULL);
		...
		case NODE_FD:
		{
			if(!fd_cache_created)
			{
				ion_fd_usage_cachep = kmem_cache_create("fd_record",sizeof(ion_fd_usage_record_t),0,0,NULL);	
		...
		case NODE_CLIENT:
		{
			...
			{
				ion_client_usage_cachep = kmem_cache_create("client_record",sizeof(ion_client_usage_record_t),0,0,NULL);
		...
		case NODE_MMAP:
		{
			...
			{
				ion_address_usage_cachep = kmem_cache_create("address_record",sizeof(ion_address_usage_record_t),0,0,NULL);
	...
	//ION_DEBUG_LOG(ION_DEBUG_INFO, "  [%d][%d][FUNCTION(%d)_%d][record_ion_info]  \n",record_param->pid,record_param->group_id,from_kernel,record_param->action);
        {
			...
			case ION_FUNCTION_IMPORT:
			{
				...
				ION_DEBUG_LOG(ION_DEBUG_TRACE, "    [%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)]input buffer: 0x%x fd %d\n",record_param->pid,record_param->group_id,from_kernel,(unsigned int)record_param->buffer,record_param->fd);
                                mutex_lock(&process_lifecycle_mutex);
                                process_record  = search_process_in_list(record_param->pid,process_created_list);
                                mutex_unlock(&process_lifecycle_mutex);
                                if(process_record == NULL)
				 ...
				 //move fd node from allocate list into free list
                                ...
                                {
                                        	ION_DEBUG_LOG(ION_DEBUG_ERROR, "    [%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)]can't found corresponding fd usage record client 0x%x ",record_param->pid,record_param->group_id,from_kernel,(unsigned int)record_param->client);
                                        break;

                                }
				 //move fd record to free list
                                 ION_DEBUG_LOG(ION_DEBUG_TRACE, "    [%d][%d][ION_FUNCTION_SHARE_CLOSE(%d)] DONE \n",record_param->pid,record_param->group_id,from_kernel);
                                break;
                        }
                        default:
