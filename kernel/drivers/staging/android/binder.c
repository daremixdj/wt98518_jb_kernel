		...
		struct binder_transaction *bt, int step)
{
	...
	size = MAX_ENG_TRANS_LOG_BUFF_LEN;//ARRAY_SIZE((t_log->entry));// build error
	if (t_log->full || (!t_log->full && t_log->next)) {
		i = t_log->next ? (t_log->next - 1) : (size - 1); 
		for ( ; i >= 0; i--) {
			if (t_log->entry[i].debug_id == bt->debug_id) {
				if (step == 1)
					do_posix_clock_monotonic_gettime(&t_log->entry[i].readstamp);
				else if (step == 2)
					do_posix_clock_monotonic_gettime(&t_log->entry[i].endstamp);
				find = true;
				break;
			}
		}
	...
	if ((false == find) && t_log->full && t_log->next) {
		for (i = size - 1; i >= t_log->next; i--) {
			if (t_log->entry[i].debug_id == bt->debug_id) {
				if (step == 1)
					do_posix_clock_monotonic_gettime(&t_log->entry[i].readstamp);
				else if (step == 2)
...
static int binder_proc_show(struct seq_file *m, void *unused)
{
	...
	if (find == true)
		...
		printk(KERN_INFO "binder: show proc addr 0x%p exist\n", proc);
#endif
	if (do_lock)
		binder_unlock(__func__);
	return 0;
