...
static int device_prepare_index = 0;
...
static int device_prepare(struct device *dev, pm_message_t state)
{
	...
	if (dev->pm_domain) {
#ifdef LOG
		printk(KERN_DEBUG "[%d] power domain device_prepare\n",device_prepare_index);
		if (dev->driver)
			if(dev->driver->name)
				printk(KERN_DEBUG "dev->driver->name=%s\n", dev->driver->name);
#endif
...
#ifdef LOG
		printk(KERN_DEBUG "[%d] type device_prepare\n",device_prepare_index);
		if (dev->driver)
			if(dev->driver->name)
				printk(KERN_DEBUG "dev->driver->name=%s\n", dev->driver->name);
#endif
...
#ifdef LOG
		printk(KERN_DEBUG "[%d] class device_prepare\n",device_prepare_index);
		if (dev->driver)
			if(dev->driver->name)
				printk(KERN_DEBUG "dev->driver->name=%s\n", dev->driver->name);
#endif
...
#ifdef LOG
		printk(KERN_DEBUG "[%d] bus device_prepare\n",device_prepare_index);
		if (dev->driver)
			if(dev->driver->name)
				printk(KERN_DEBUG "dev->driver->name=%s\n", dev->driver->name);
#endif
	...
	if (!callback && dev->driver && dev->driver->pm) {
#ifdef LOG
		printk(KERN_DEBUG "[%d] driver device_prepare\n",device_prepare_index);
		if (dev->driver)
			if(dev->driver->name)
				printk(KERN_DEBUG "dev->driver->name=%s\n", dev->driver->name);
#endif
