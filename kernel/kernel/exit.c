...
#ifdef CONFIG_SCHEDSTATS
	{
	struct task_struct *tg = tsk->group_leader;
	if( !thread_group_empty(tsk) ){
		printk(KERN_DEBUG "[%d:%d:%s] exit Done (0x%p/0x%p)(#%d/%s) leader: state(%d) exit_state(%d) tsk: signal->flags=0x%x group_exit_code=0x%x\n",
		   tsk->tgid, tsk->pid, tsk->comm, 
		   tsk, tg, get_nr_threads(tsk), thread_group_empty(tsk) ? "empty" : "not empty",
		   tg->state, tg->exit_state,
		   tsk->signal->flags, tsk->signal->group_exit_code);
	}
	}
#endif
#endif
