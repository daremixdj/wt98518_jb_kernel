...
static int sched_rt_runtime_exceeded(struct rt_rq *rt_rq)
{
	...
	if (rt_rq->rt_time > runtime) {
		...
		printk_sched("sched: cpu=%d rt_time %llu <-> runtime"
			     ...
			     rt_rq->rq->cpu, rt_rq->rt_time, runtime_pre, runtime, 
			     per_cpu(exec_delta_time, rt_rq->rq->cpu),
			     per_cpu(clock_task, rt_rq->rq->cpu),
			     per_cpu(exec_start, rt_rq->rq->cpu));
		/*
		...
		//	}
#ifdef CONFIG_MT_RT_SCHED_CRIT
			trace_sched_rt_crit(rt_rq->rq->cpu, rt_rq->rt_throttled);
#endif
...
static void update_curr_rt(struct rq *rq)
{
	...
	per_cpu(exec_start, rq->cpu) = curr->se.exec_start;
