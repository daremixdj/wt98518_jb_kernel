...
int show_interrupts(struct seq_file *p, void *v)
{
	...
	if (i == 0) {
        ...
        seq_printf(p, "%15s:%10s:%10s:%10s:%10s:%10s", "Total(ns)", "act_counts", "lv1#","lv2#","lv3#","max");
#endif
		seq_putc(p, '\n');
	}
	...
	if (action) {
        ...
        seq_printf(p, ":%10lu:%10lu:%10lu", action->count_100us, action->count_500us, action->count_1ms);
        seq_printf(p, ":%10llu", action->dur_max);
